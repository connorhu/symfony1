 ...
 * @coversNothing
 */
class sfWidgetFormDateTest extends TestCase
{
    ...
    public function testTodoMigrate()
    {
        ...
        // pre-epoch date
        $this->diag('pre-epoch date');
        $years = range(1901, 1920);
        $w1 = new sfWidgetFormDate(array('years' => array_combine($years, $years)));
        $dom->loadHTML($w1->render('foo', '1910-01-15'));
        $css = new sfDomCssSelector($dom);

        $this->is($css->matchSingle('#foo_year option[selected="selected"]')->getValue(), '1910', '->render() renders a select tag for the year');
        $this->is($css->matchSingle('#foo_month option[selected="selected"]')->getValue(), '01', '->render() renders a select tag for the month');
        $this->is($css->matchSingle('#foo_day option[selected="selected"]')->getValue(), '15', '->render() renders a select tag for the day');
        ...
        $dom->loadHTML($w->render('foo', $year.'-10-15'));
        $css = new sfDomCssSelector($dom);
        $this->is(count($css->matchAll('#foo_year option')->getNodes()), 11, '->render() renders a select tag for the 10 years around the current one');
        $this->is(count($css->matchAll('#foo_month option')->getNodes()), 12, '->render() renders a select tag for the 12 months in a year');
        $this->is(count($css->matchAll('#foo_day option')->getNodes()), 31, '->render() renders a select tag for the 31 days in a month');
        ...
        $dom->loadHTML($w->render('foo', $year.'-10-15'));
        $css = new sfDomCssSelector($dom);
        $this->is($css->matchSingle('#foo_year option')->getNode()->nodeValue, 'YEAR', '->configure() can change the empty values');
        $this->is($css->matchSingle('#foo_month option')->getNode()->nodeValue, 'MONTH', '->configure() can change the empty values');
        $this->is($css->matchSingle('#foo_day option')->getNode()->nodeValue, 'DAY', '->configure() can change the empty values');
        $w->setOption('empty_values', array('year' => '', 'month' => '', 'day' => ''));

        // format option
        $this->diag('format option');
        $this->is($css->matchSingle('#foo_day')->getNode()->nextSibling->nodeValue, '/', '->render() renders 3 selects with a default / as a separator');
        $this->like($css->matchSingle('#foo_month')->getNode()->nextSibling->nodeValue, '#^/#', '->render() renders 3 selects with a default / as a separator');

        $w->setOption('format', '%month%#%day%#%year%');
        $dom->loadHTML($w->render('foo', $year.'-10-15'));
        $css = new sfDomCssSelector($dom);
        $this->is($css->matchSingle('#foo_day')->getNode()->nextSibling->nodeValue, '#', '__construct() can change the default date format');
        $this->like($css->matchSingle('#foo_month')->getNode()->nextSibling->nodeValue, '/^#/', '__construct() can change the default date format');

        $w->setOption('format', '%day%/%month%/%year%');
        $dom->loadHTML($w->render('foo', $year.'-10-15'));
        $css = new sfDomCssSelector($dom);
        $this->is($css->matchSingle('select')->getNode()->getAttribute('name'), 'foo[day]', '__construct() can change the default date format');

        // days / months / years options
        $this->diag('days / months / years options');
        ...
        $dom->loadHTML($w->render('foo', $year.'-10-15'));
        $css = new sfDomCssSelector($dom);
        $this->is(count($css->matchAll('#foo_year option')->getNodes()), 5, '__construct() can change the default array used for years');
        $this->is(count($css->matchAll('#foo_month option')->getNodes()), 4, '__construct() can change the default array used for months');
        $this->is(count($css->matchAll('#foo_day option')->getNodes()), 3, '__construct() can change the default array used for days');
        ...
        $this->is(count($css->matchAll('#id_foo_day')), 1, '->render() uses the id_format');
        $this->is(count($css->matchAll('#id_foo_month')), 1, '->render() uses the id_format');
        $this->is(count($css->matchAll('#id_foo_year')), 1, '->render() uses the id_format');
    }
}
