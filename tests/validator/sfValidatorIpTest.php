 ...
 * @coversNothing
 */
class sfValidatorIpTest extends TestCase
{
    ...
    public function testTodoMigrate()
    {
        ...
        $this->diag('testing against empty IP');
        ...
        foreach (array(null, '') as $ip) {
            try {
                $v->clean($ip);
                $this->pass('Empty IP is considered as valid');
            } catch (sfValidatorError $e) {
                $this->fail('Empty IP is considered as valid');
            }
        }

        $this->diag('testing against valid IP all');
        foreach (array_merge($validV4Ips, $validV6Ips) as $ip) {
            try {
                $v->clean($ip);
                $this->pass(sprintf('%s is considered as valid ip', $ip));
            } catch (sfValidatorError $e) {
                $this->fail(sprintf('%s is considered as valid ip', $ip));
            }
        }

        foreach (array(sfValidatorIp::IP_V4 => $validV4Ips, sfValidatorIp::IP_V6 => $validV6Ips) as $version => $ips) {
            $this->diag('testing against valid IP V'.$version);
        ...
        $this->diag('testing against invalid IP all');
        foreach (array_merge($invalidV4Ips, $invalidV6Ips) as $ip) {
            try {
                $v->clean($ip);
                $this->fail(sprintf('%s is not considered as valid ip', $ip));
            } catch (sfValidatorError $e) {
                $this->pass(sprintf('%s is not considered as valid ip', $ip));
            }
        }

        foreach (array(sfValidatorIp::IP_V4 => $invalidV4Ips, sfValidatorIp::IP_V6 => $invalidV6Ips) as $version => $ips) {
            $this->diag('testing against invalid IP V'.$version);
            $v->setOption('version', $version);
            foreach ($ips as $ip) {
                try {
                    $v->clean($ip);
        ...
        $this->diag('testing against invalid private IP all');
        ...
        $this->diag('testing against invalid reserved IP all');
        ...
        $this->diag('testing against invalid public IP all');
