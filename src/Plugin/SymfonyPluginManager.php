...
use Symfony1\Components\Util\ClassManipulator;
use PEAR_PackageFile_v2_rw;
use function is_dir;
 ...
 * @version SVN: $Id$
 */
class SymfonyPluginManager extends PluginManager
{
    /**
    ...
    public function initialize(EventDispatcher $dispatcher, PearEnvironment $environment)
    {
        ...
        if (!$environment->getOption('web_dir')) {
            throw new PluginException('You must provide a "web_dir" option.');
        }
    }
    /**
    ...
    public function installWebContent($plugin, $sourceDirectory)
    {
        ...
        if (is_dir($webDir)) {
            $this->dispatcher->notify(new Event($this, 'application.log', array('Installing web data for plugin')));
    ...
    public function uninstallWebContent($plugin)
    {
        ...
        if (is_dir($targetDir)) {
            $this->dispatcher->notify(new Event($this, 'application.log', array('Uninstalling web data for plugin')));
            $filesystem = new Filesystem();
            if (is_link($targetDir)) {
                $filesystem->remove($targetDir);
            } else {
                $filesystem->remove(Finder::type('any')->in($targetDir));
                $filesystem->remove($targetDir);
            }
        }
    }
    ...
    */
    ...
    {
        if (!$configDir) {
            throw new PluginException('You must provide a "config_dir" option.');
        }
        $manipulator = ClassManipulator::fromFile($configDir . '/ProjectConfiguration.class.php');
    ...
    */
    ...
    protected function registerSymfonyPackage()
    {
        ...
        if (!$this->environment->getRegistry()->addPackage2($symfony)) {
            throw new PluginException('Unable to register the symfony package');
        }
    }
    /**
...
class_alias(SymfonyPluginManager::class, 'sfSymfonyPluginManager', false);