 ...
 * @version SVN: $Id$
 */
class PearRestPlugin extends PearRest11
{
    protected $config;
    protected $rest10;
    protected $restBase;
    ...
    public function __construct(PEAR_Config $config, $options = array())
    {
        ...
        $this->config = $config;
        $this->rest10 = new PearRest10($config, $options);
    }
    /**
    ...
    public function getPluginLicense($plugin, $version)
    {
        ...
        if (PEAR::isError($info)) {
            throw new PluginRestException(sprintf('Unable to get plugin licence information for plugin "%s": %s', $plugin, $info->getMessage()));
        }
        if (null === $info) {
    ...
    public function getPluginVersions($plugin, $stability = null)
    {
        ...
        if (PEAR::isError($allreleases)) {
            throw new PluginRestException(sprintf('Unable to get information for plugin "%s": %s', $plugin, $allreleases->getMessage()));
        }
        if (!isset($allreleases['r']) || (isset($allreleases['r']) && !is_array($allreleases['r']) || !count($allreleases['r']))) {
            throw new PluginRestException(sprintf('No release available for plugin "%s"', $plugin));
        }
        ...
        if (!count($versions)) {
            throw new PluginException(sprintf('No release available for plugin "%s" in state "%s"', $plugin, $stability));
        }
        return $versions;
    }
    /**
    ...
    public function getPluginDependencies($plugin, $version)
    {
        ...
        if (PEAR::isError($dependencies)) {
            throw new PluginRestException(sprintf('Unable to get dependencies information for plugin "%s": %s', $plugin, $dependencies->getMessage()));
        }
    ...
    public function getPluginDownloadURL($plugin, $version, $stability)
    {
        ...
        if ($installed >= $version) {
            throw new PluginException(sprintf('Plugin "%s" version "%s" is already installed (you tried to install version "%s")', $plugin, $installed, $version));
        }
        $info = $this->getDownloadURL($this->restBase, array('channel' => $this->channel, 'package' => $plugin, 'version' => $version), $stability, $installed);
        if (PEAR::isError($info)) {
            throw new PluginRestException(sprintf('Unable to get download information for plugin "%s | %s | %s": %s', $plugin, $version, $stability, $info->getMessage()));
        }
        if (!isset($info['url'])) {
            throw new PluginRestException(sprintf('Plugin "%s" cannot be installed (No URL found)', $plugin));
        }
    ...
    protected function getRESTBase($channelName)
    {
        ...
        if (PEAR::isError($channel)) {
            throw new PluginException(sprintf('Unable to initialize channel "%s"', $channel->getMessage()));
        }
        $mirror = $this->config->get('preferred_mirror', null, $channelName);
        if (!$channel->supportsREST($mirror)) {
            throw new PluginRestException(sprintf('The channel "%s" does not support the REST protocol', $channelName));
        }
...
class_alias(PearRestPlugin::class, 'sfPearRestPlugin', false);