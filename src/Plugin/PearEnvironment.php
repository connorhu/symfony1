    ...
    */
    public function initialize(EventDispatcher $dispatcher, $options = array())
    {
        ...
        if (!isset($options['plugin_dir'])) {
            throw new ConfigurationException('You must provide a "plugin_dir" option.');
        }
        if (!isset($options['cache_dir'])) {
            throw new ConfigurationException('You must provide a "cache_dir" option.');
        }
        ...
        $this->rest = new PearRestPlugin($this->config, array('base_class' => $options['rest_base_class']));
    ...
    public function registerChannel($channel, $isDefault = false)
    {
        ...
        if (!$this->registry->channelExists($channel, true)) {
            ...
            if (!$downloader->discover($channel)) {
                throw new PluginException(sprintf('Unable to register channel "%s"', $channel));
            }
        }
        if ($isDefault) {
    ...
    {
        $this->config = $GLOBALS['_PEAR_Config_instance'] = new PearConfig();
    ...
    protected function initializeFrontend()
    {
        ...
        if (PEAR::isError($this->frontend)) {
            throw new PluginException(sprintf('Unable to initialize PEAR Frontend object: %s', $this->frontend->getMessage()));
        }
    ...
    protected function initializeRegistry()
    {
        ...
        if (PEAR::isError($this->registry)) {
            throw new PluginException(sprintf('Unable to initialize PEAR registry: %s', $this->registry->getMessage()));
        }
    }
}
class_alias(PearEnvironment::class, 'sfPearEnvironment', false);