...
use Symfony1\Components\Widget\WidgetFormSchemaDecorator;
use Symfony1\Components\Validator\ValidatorBase;
use Symfony1\Components\Validator\ValidatorSchema;
use Symfony1\Components\Validator\ValidatorError;
use Symfony1\Components\Validator\ValidatorErrorSchema;
use Exception;
use InvalidArgumentException;
use LogicException;
use Symfony1\Components\Validator\ValidatorPass;
use Symfony1\Components\Validator\ValidatorAnd;
use Symfony1\Components\Widget\WidgetForm;
use Symfony1\Components\Validator\ValidatorCSRFToken;
use Symfony1\Components\Widget\WidgetFormInputHidden;
...
class Form implements ArrayAccess, Iterator, Countable
{
    ...
    /**
     * @var (Widget[] | WidgetFormSchema | WidgetFormSchemaDecorator[])
     */
    protected $widgetSchema;
    /**
     * @var (ValidatorBase[] | ValidatorSchema)
     */
    protected $validatorSchema;
    /**
     * @var (ValidatorError[] | ValidatorErrorSchema)
     */
    protected $errorSchema;
    /**
     * @var (FormFieldSchema | null)
     */
    ...
    public function __construct($defaults = array(), $options = array(), $CSRFSecret = null)
    {
        ...
        $this->widgetSchema = new WidgetFormSchema();
        $this->errorSchema = new ValidatorErrorSchema($this->validatorSchema);
    ...
    public function bindEmbeddedForms(array $taintedValues = null, array $taintedFiles = null)
    {
        foreach ($this->embeddedForms as $name => $form) {
            ...
            // bind
            $form->bind(isset($taintedValues[$name]) ? $taintedValues[$name] : array(), isset($taintedFiles[$name]) ? $taintedFiles[$name] : array());
            ...
            $this->widgetSchema[$name] = new WidgetFormSchemaDecorator($widgetSchema, $decorator);
     ...
     *
     * @return ValidatorErrorSchema A sfValidatorErrorSchema instance
     */
     ...
     * @param Form $form A sfForm instance
     * @param string $decorator A HTML decorator for the embedded form
     */
    public function embedForm($name, Form $form, $decorator = null)
    {
        $name = (string) $name;
        ...
        $this->widgetSchema[$name] = new WidgetFormSchemaDecorator($widgetSchema, $decorator);
        $this->validatorSchema[$name] = new ValidatorPass();
        // keep widgetSchema synchronized
        $form->setWidgetSchema($this->widgetSchema[$name]->getWidget());
        $this->resetFormFields();
    }
     ...
     *
     * @return Form[] An array of embedded forms
     */
    ...
    *
    * @param Form $form The sfForm instance to merge with current form
    *
    ...
    */
     ...
     *
     * @param ValidatorBase $validator A validator to be merged
     */
    public function mergePreValidator(ValidatorBase $validator = null)
    {
        if (null === $validator) {
            return;
        }
        if (null === $this->validatorSchema->getPreValidator()) {
            ...
            $this->validatorSchema->setPreValidator(new ValidatorAnd(array($this->validatorSchema->getPreValidator(), $validator)));
        }
    }
    /**
     ...
     * @param ValidatorBase $validator A validator to be merged
     */
    public function mergePostValidator(ValidatorBase $validator = null)
    {
        if (null === $validator) {
            return;
        }
        if (null === $this->validatorSchema->getPostValidator()) {
            ...
            $this->validatorSchema->setPostValidator(new ValidatorAnd(array($this->validatorSchema->getPostValidator(), $validator)));
        }
    }
    /**
    ...
    {
        $this->setValidatorSchema(new ValidatorSchema($validators));
        return $this;
    }
    /**
     ...
     * @return Form The current form instance
     */
    public function setValidator($name, ValidatorBase $validator)
    {
        if (isset($this->embeddedForms[$name])) {
     ...
     * @param string $name The field name
     *
     * @return ValidatorBase $validator The validator
     */
     ...
     *
     * @param ValidatorSchema $validatorSchema A sfValidatorSchema instance
     *
     * @return Form The current form instance
     */
    public function setValidatorSchema(ValidatorSchema $validatorSchema)
    {
     ...
     *
     * @return ValidatorSchema A sfValidatorSchema instance
     */
    ...
    {
        $this->setWidgetSchema(new WidgetFormSchema($widgets));
        return $this;
    }
    /**
     ...
     *
     * @param string $name The field name
     * @param WidgetForm $widget The widget
     *
     * @return Form The current form instance
     */
    public function setWidget($name, WidgetForm $widget)
    {
     ...
     * @param string $name The field name
     *
     * @return WidgetForm $widget The widget
     */
     ...
     *
     * @param WidgetFormSchema $widgetSchema A sfWidgetFormSchema instance
     *
     * @return Form The current form instance
     */
    public function setWidgetSchema(WidgetFormSchema $widgetSchema)
    {
     ...
     *
     * @return WidgetFormSchema A sfWidgetFormSchema instance
     */
    ...
    public function addCSRFProtection($secret = null)
    {
        ...
        $this->validatorSchema[self::$CSRFFieldName] = new ValidatorCSRFToken(array('token' => $token));
        $this->widgetSchema[self::$CSRFFieldName] = new WidgetFormInputHidden();
    ...
    */
     ...
     *
     * @return (FormField | FormFieldSchema) A form field instance
     */
    #[ReturnTypeWillChange]
    public function offsetGet($name)
    {
        if (!isset($this->formFields[$name])) {
            ...
            if ($this->isBound) {
                ...
                $value = $widget instanceof WidgetFormSchema ? $widget->getDefaults() : $widget->getDefault();
            }
            $class = $widget instanceof WidgetFormSchema ? 'sfFormFieldSchema' : 'sfFormField';
    ...
    {
        unset($this->widgetSchema[$offset], $this->validatorSchema[$offset], $this->defaults[$offset], $this->taintedValues[$offset], $this->values[$offset], $this->embeddedForms[$offset]);
        $this->resetFormFields();
    }
    /**
     ...
     *
     * @return FormFieldSchema A sfFormFieldSchema instance
     */
    public function getFormFieldSchema()
    {
        if (null === $this->formFieldSchema) {
            ...
            $this->formFieldSchema = new FormFieldSchema($this->widgetSchema, null, null, $values, $this->errorSchema);
        }
    ...
    public function getErrors()
    {
        ...
        foreach ($this as $name => $field) {
            if (!$field instanceof FormFieldSchema && $field->hasError()) {
    ...
    protected static function fixPhpFilesArray($data)
    {
        ...
        foreach (array_keys($data['name']) as $key) {
            $files[$key] = self::fixPhpFilesArray(array('error' => $data['error'][$key], 'name' => $data['name'][$key], 'type' => $data['type'][$key], 'tmp_name' => $data['tmp_name'][$key], 'size' => $data['size'][$key]));
        }
        return $files;
    }
    /**
...
class_alias(Form::class, 'sfForm', false);