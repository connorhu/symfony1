...
abstract class Cache
{
    ...
    */
    ...
    {
        $this->options = array_merge(array('automatic_cleaning_factor' => 1000, 'lifetime' => 86400, 'prefix' => md5(__DIR__)), $options);
     ...
     */
    public abstract function set($key, $data, $lifetime = null);
    /**
     ...
     * @see patternToRegexp
     */
    public abstract function removePattern($pattern);
    /**
    * Cleans the cache.
    *
    ...
    * @return bool true if no problem
    */
    public abstract function clean($mode = self::ALL);
    /**
     ...
     */
    public abstract function getTimeout($key);
    /**
     ...
     */
    public abstract function getLastModified($key);
    /**
    ...
    */
    ...
    {
        $regexp = str_replace(array('\\*\\*', '\\*'), array('.+?', '[^' . preg_quote(Cache::SEPARATOR, '#') . ']+'), preg_quote($pattern, '#'));
        return '#^' . $regexp . '$#';
    }
}
class_alias(Cache::class, 'sfCache', false);