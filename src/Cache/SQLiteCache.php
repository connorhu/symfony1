...
use function time;
use function sqlite_escape_string;
...
use function mkdir;
use function touch;
use function sqlite_error_string;
...
class SQLiteCache extends Cache
{
    ...
    public function initialize($options = array())
    {
        if (!extension_loaded('SQLite') && !extension_loaded('pdo_SQLite')) {
            throw new ConfigurationException('sfSQLiteCache class needs "sqlite" or "pdo_sqlite" extension to be loaded.');
        }
        parent::initialize($options);
        if (!$this->getOption('database')) {
            throw new InitializationException('You must pass a "database" option to initialize a sfSQLiteCache object.');
        }
    ...
    {
        if ($this->isSqLite3()) {
            $res = $this->dbh->exec('DELETE FROM cache' . (Cache::OLD == $mode ? sprintf(" WHERE timeout < '%s'", time()) : ''));
            if ($res) {
                return (bool) $this->dbh->changes();
            }
            return false;
        }
        return (bool) $this->dbh->query('DELETE FROM cache' . (Cache::OLD == $mode ? sprintf(" WHERE timeout < '%s'", time()) : ''))->numRows();
    }
    /**
     * @see sfCache
     */
    ...
    protected function setDatabase($database)
    {
        ...
        if ($this->isSqLite3()) {
            ...
            if ('not an error' !== ($errmsg = $this->dbh->lastErrorMsg())) {
                throw new CacheException(sprintf('Unable to connect to SQLite database: %s.', $errmsg));
            }
        } else {
    ...
    {
        $statements = array('CREATE TABLE [cache] (
        [key] VARCHAR(255),
        [data] LONGVARCHAR,
        [timeout] TIMESTAMP,
...
class_alias(SQLiteCache::class, 'sfSQLiteCache', false);