...
class XCacheCache extends Cache
{
    ...
    public function initialize($options = array())
    {
        ...
        if (!function_exists('xcache_set')) {
            throw new InitializationException('You must have XCache installed and enabled to use sfXCacheCache class.');
        }
        if (!ini_get('xcache.var_size')) {
            throw new InitializationException('You must set the "xcache.var_size" variable to a value greater than 0 to use sfXCacheCache class.');
        }
    }
    /**
     * @see sfCache
    ...
    public function set($key, $data, $lifetime = null)
    {
        ...
        $set = array('timeout' => time() + $lifetime, 'data' => $data, 'ctime' => time());
    ...
    protected function checkAuth()
    {
        if (ini_get('xcache.admin.enable_auth')) {
            throw new ConfigurationException('To use all features of the "sfXCacheCache" class, you must set "xcache.admin.enable_auth" to "Off" in your php.ini.');
        }
    }
}
class_alias(XCacheCache::class, 'sfXCacheCache', false);