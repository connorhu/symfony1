...
use Symfony1\Components\Debug\WebDebug;
 ...
 * @version SVN: $Id$
 */
class WebDebugLogger extends VarLogger
{
    /**
     * @var Context
     */
    ...
    */
    public function handlePhpError($errno, $errstr, $errfile, $errline, $errcontext = array())
    {
        ...
        switch ($errno) {
            case E_STRICT:
                $this->dispatcher->notify(new Event($this, 'application.log', array('priority' => Logger::ERR, sprintf($message, 'Strict notice'))));
                break;
            case E_NOTICE:
                $this->dispatcher->notify(new Event($this, 'application.log', array('priority' => Logger::NOTICE, sprintf($message, 'Notice'))));
                break;
            case E_WARNING:
                $this->dispatcher->notify(new Event($this, 'application.log', array('priority' => Logger::WARNING, sprintf($message, 'Warning'))));
                break;
            case E_RECOVERABLE_ERROR:
                $this->dispatcher->notify(new Event($this, 'application.log', array('priority' => Logger::ERR, sprintf($message, 'Error'))));
                break;
        }
        return false;
        // do not prevent default error handling
    }
    /**
     * Listens for the context.load_factories event.
     */
    public function listenForLoadFactories(Event $event)
    {
        $path = sprintf('%s/%s/images', $event->getSubject()->getRequest()->getRelativeUrlRoot(), Config::get('sf_web_debug_web_dir'));
        $path = str_replace('//', '/', $path);
        $this->webDebug = new $this->webDebugClass($this->dispatcher, $this, array('image_root_path' => $path, 'request_parameters' => $event->getSubject()->getRequest()->getParameterHolder()->getAll()));
    }
    /**
     * Listens to the response.filter_content event.
     *
     * @param Event $event The sfEvent instance
     * @param string $content The response content
     *
     * @return string The filtered response content
     */
    public function filterResponseContent(Event $event, $content)
    {
        if (!Config::get('sf_web_debug')) {
            return $content;
        }
        ...
        $messages = array();
        foreach (TimerManager::getTimers() as $name => $timer) {
            $messages[] = sprintf('%s %.2f ms (%d)', $name, $timer->getElapsedTime() * 1000, $timer->getCalls());
        }
        $this->dispatcher->notify(new Event($this, 'application.log', $messages));
        ...
        if (null === $this->webDebug || !$this->context->has('request') || !$this->context->has('response') || !$this->context->has('controller') || $request->isXmlHttpRequest() || false === strpos($response->getContentType(), 'html') || '3' == substr($response->getStatusCode(), 0, 1) || View::RENDER_CLIENT != $this->context->getController()->getRenderMode() || $response->isHeaderOnly()) {
            return $content;
        }
...
class_alias(WebDebugLogger::class, 'sfWebDebugLogger', false);