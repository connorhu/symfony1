...
namespace Symfony1\Components\User;

use ArrayAccess;
use Symfony1\Components\Util\NamespacedParameterHolder;
...
class User implements ArrayAccess
{
     ...
     * @param array $options
     */
    public function __construct(EventDispatcher $dispatcher, Storage $storage, $options = array())
    {
     ...
     *
     * @throws Exception If the calls fails
     */
    public function __call($method, $arguments)
    {
        $event = $this->dispatcher->notifyUntil(new Event($this, 'user.method_not_found', array('method' => $method, 'arguments' => $arguments)));
    ...
    * @param Storage $storage an sfStorage instance
    * @param array $options an associative array of options
    */
    public function initialize(EventDispatcher $dispatcher, Storage $storage, $options = array())
    {
        ...
        $this->attributeHolder = new NamespacedParameterHolder(self::ATTRIBUTE_NAMESPACE);
        ...
        if ($this->options['use_flash'] && ($names = $this->attributeHolder->getNames('symfony/user/sfUser/flash'))) {
            if ($this->options['logging']) {
                $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Flag old flash messages ("%s")', implode('", "', $names)))));
            }
    ...
    public function shutdown()
    {
        ...
        if ($this->options['use_flash'] && ($names = $this->attributeHolder->getNames('symfony/user/sfUser/flash/remove'))) {
            if ($this->options['logging']) {
                $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Remove old flash messages ("%s")', implode('", "', $names)))));
            }
            foreach ($names as $name) {
...
class_alias(User::class, 'sfUser', false);