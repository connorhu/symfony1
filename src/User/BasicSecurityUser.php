 ...
 * @version SVN: $Id$
 */
class BasicSecurityUser extends User implements SecurityUser
{
            ...
            foreach ($this->credentials as $key => $value) {
                if ($credential == $value) {
                    if ($this->options['logging']) {
                        $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Remove credential "%s"', $credential))));
                    }
    ...
    public function addCredentials()
    {
        ...
        if ($this->options['logging']) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Add credential(s) "%s"', implode(', ', $credentials)))));
        }
        $added = false;
    ...
    {
        if ($this->options['logging']) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('User is %sauthenticated', true === $authenticated ? '' : 'not '))));
        }
        if ((bool) $authenticated !== $this->authenticated) {
            ...
            $this->dispatcher->notify(new Event($this, 'user.change_authentication', array('authenticated' => $this->authenticated)));
            $this->storage->regenerate(true);
        }
    }
    ...
    public function initialize(EventDispatcher $dispatcher, Storage $storage, $options = array())
    {
        ...
        if (null === $this->authenticated) {
            ...
            if (false !== $timeout && null !== $this->lastRequest && time() - $this->lastRequest >= $timeout) {
                if ($this->options['logging']) {
                    $this->dispatcher->notify(new Event($this, 'application.log', array('Automatic user logout due to timeout')));
                }
                $this->setTimedOut();
...
class_alias(BasicSecurityUser::class, 'sfBasicSecurityUser', false);