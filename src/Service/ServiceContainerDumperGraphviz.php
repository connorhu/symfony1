 ...
 * @version SVN: $Id$
 */
class ServiceContainerDumperGraphviz extends ServiceContainerDumper
{
    protected $nodes;
    protected $edges;
    /* array */
    ...
    */
    ...
    {
        $this->options = array('graph' => array('ratio' => 'compress'), 'node' => array('fontsize' => 11, 'fontname' => 'Arial', 'shape' => 'record'), 'edge' => array('fontsize' => 9, 'fontname' => 'Arial', 'color' => 'grey', 'arrowhead' => 'open', 'arrowsize' => 0.5), 'node.instance' => array('fillcolor' => '#9999ff', 'style' => 'filled'), 'node.definition' => array('fillcolor' => '#eeeeee'), 'node.missing' => array('fillcolor' => '#ff9999', 'style' => 'filled'));
    ...
    protected function findEdges($id, $arguments, $required, $name)
    {
        ...
        foreach ($arguments as $argument) {
            if (is_object($argument) && $argument instanceof ServiceParameter) {
    ...
    protected function findNodes()
    {
        ...
        foreach ($container->getServiceDefinitions() as $id => $definition) {
            ...
            $container->setServiceDefinition($id, new ServiceDefinition('stdClass'));
        }
...
class_alias(ServiceContainerDumperGraphviz::class, 'sfServiceContainerDumperGraphviz', false);