 ...
 * @version SVN: $Id$
 */
class ServiceContainerDumperPhp extends ServiceContainerDumper
{
    /**
    ...
    */
    ...
    {
        $options = array_merge(array('class' => 'ProjectServiceContainer', 'base_class' => 'sfServiceContainer'), $options);
        return $this->startClass($options['class'], $options['base_class']) . $this->addConstructor() . $this->addServices() . $this->addDefaultParametersMethod() . $this->endClass();
    }
...
EOF;
    ...
    protected function addServiceConfigurator($id, $definition)
    {
        ...
        if (is_array($callable)) {
            if (is_object($callable[0]) && $callable[0] instanceof ServiceReference) {
    ...
    {
        $name = ServiceContainer::camelize($id);
        $code = <<<EOF
...
EOF;
        $code .= $this->addServiceInclude($id, $definition) . $this->addServiceShared($id, $definition) . $this->addServiceInstance($id, $definition) . $this->addServiceMethodCalls($id, $definition) . $this->addServiceConfigurator($id, $definition) . $this->addServiceReturn($id, $definition);
        return $code;
    }
    protected function addServiceAlias($alias, $id)
    {
        $name = ServiceContainer::camelize($alias);
        return <<<EOF
...
EOF;
    ...
    protected function exportParameters($parameters, $indent = 6)
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            } elseif ($value instanceof ServiceReference) {
                $value = sprintf("new sfServiceReference('%s')", $value);
            } elseif ($value instanceof ServiceParameter) {
...
EOF;
    ...
    protected function dumpValue($value)
    {
        ...
        if (is_object($value) && $value instanceof ServiceReference) {
            return $this->getServiceCall((string) $value);
        }
        if (is_object($value) && $value instanceof ServiceParameter) {
...
class_alias(ServiceContainerDumperPhp::class, 'sfServiceContainerDumperPhp', false);