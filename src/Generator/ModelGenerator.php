 ...
 * @version SVN: $Id$
 */
abstract class ModelGenerator extends Generator
{
    /**
    ...
    public function generate($params = array())
    {
        ...
        if (!is_dir($themeDir)) {
            throw new ConfigurationException(sprintf('The theme "%s" does not exist.', $theme));
        }
        // configure the model
        $this->configure();
        $this->configuration = $this->loadConfiguration();
        // generate files
        $this->generatePhpFiles($this->generatedModuleName, Finder::type('file')->relative()->in($themeDir));
        // move helper file
    ...
    {
        return isset($this->params['singular']) ? $this->params['singular'] : Inflector::underscore($this->getModelClass());
    }
    /**
    ...
    public function getRetrieveByPkParamsForAction($indent)
    {
        ...
        foreach ($this->getPrimaryKeys() as $pk) {
            $params[] = sprintf("\$request->getParameter('%s')", Inflector::underscore($pk));
        }
    ...
    public function getPrimaryKeyUrlParams($prefix = '', $full = false)
    {
        $params = array();
        foreach ($this->getPrimaryKeys() as $pk) {
            $fieldName = Inflector::underscore($pk);
            if ($full) {
    ...
    public abstract function getType($column);
     ...
     * @return string PHP code
     */
    public abstract function getColumnGetter($column, $developed = false, $prefix = '');
    public abstract function getManyToManyTables();
    /**
    ...
    {
        $action = isset($params['action']) ? $params['action'] : 'List' . Inflector::camelize($actionName);
...
EOF;
     ...
     *
     * @param ModelGeneratorConfigurationField $field The field
     *
     * @return string HTML code
     */
    ...
    protected function validateParameters($params)
    {
        foreach (array('model_class', 'moduleName') as $key) {
            if (!isset($params[$key])) {
                throw new ParseException(sprintf('sfModelGenerator must have a "%s" parameter.', $key));
            }
        }
        if (!class_exists($params['model_class'])) {
            throw new InitializationException(sprintf('Unable to generate a module for non-existent model "%s".', $params['model_class']));
        }
...
class_alias(ModelGenerator::class, 'sfModelGenerator', false);