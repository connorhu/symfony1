...
class ValidatorDate extends ValidatorBase
{
    ...
    */
    ...
    protected function doClean($value)
    {
        ...
        if (is_string($value) && ($regex = $this->getOption('date_format'))) {
            if (!preg_match($regex, $value, $match)) {
                throw new ValidatorError($this, 'bad_format', array('value' => $value, 'date_format' => $this->getOption('date_format_error') ?: $this->getOption('date_format')));
            }
            $value = $match;
        }
    ...
    protected function convertDateArrayToString($value)
    {
        ...
        $empties = (!isset($value['year']) || !$value['year'] ? 1 : 0) + (!isset($value['month']) || !$value['month'] ? 1 : 0) + (!isset($value['day']) || !$value['day'] ? 1 : 0);
        ...
        if ($this->getOption('with_time')) {
            ...
            if ($this->isValueSet($value, 'second') && (!$this->isValueSet($value, 'minute') || !$this->isValueSet($value, 'hour')) || $this->isValueSet($value, 'minute') && !$this->isValueSet($value, 'hour')) {
                throw new ValidatorError($this, 'invalid', array('value' => $value));
            }
            $clean = sprintf('%04d-%02d-%02d %02d:%02d:%02d', (int) $value['year'], (int) $value['month'], (int) $value['day'], isset($value['hour']) ? (int) $value['hour'] : 0, isset($value['minute']) ? (int) $value['minute'] : 0, isset($value['second']) ? (int) $value['second'] : 0);
        } else {
            $clean = sprintf('%04d-%02d-%02d %02d:%02d:%02d', (int) $value['year'], (int) $value['month'], (int) $value['day'], 0, 0, 0);
        }
        return $clean;
    }
...
class_alias(ValidatorDate::class, 'sfValidatorDate', false);