 ...
 * @version SVN: $Id$
 */
class ValidatorTime extends ValidatorBase
{
    /**
    ...
    */
    ...
    protected function doClean($value)
    {
        if (is_array($value)) {
            ...
            if (!preg_match($regex, $value, $match)) {
                throw new ValidatorError($this, 'bad_format', array('value' => $value, 'time_format' => $this->getOption('time_format_error') ?: $this->getOption('time_format')));
            }
    ...
    protected function convertTimeArrayToTimestamp($value)
    {
        ...
        $clean = mktime(isset($value['hour']) ? (int) $value['hour'] : 0, isset($value['minute']) ? (int) $value['minute'] : 0, isset($value['second']) ? (int) $value['second'] : 0);
        if (false === $clean) {
...
class_alias(ValidatorTime::class, 'sfValidatorTime', false);