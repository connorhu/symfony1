...
class ValidatorFile extends ValidatorBase
{
    ...
    */
    protected function configure($options = array(), $messages = array())
    {
        ...
        $this->addOption('mime_categories', array('web_images' => array('image/jpeg', 'image/pjpeg', 'image/png', 'image/x-png', 'image/gif')));
    ...
    */
    ...
    {
        if (!is_array($value)) {
            throw new ValidatorError($this, 'invalid', array('value' => (string) $value));
        }
        if (!isset($value['tmp_name'])) {
            throw new ValidatorError($this, 'invalid', array('value' => var_export($value, true)));
        }
        if (!isset($value['name'])) {
            $value['name'] = '';
        }
        ...
        switch ($value['error']) {
            case UPLOAD_ERR_INI_SIZE:
                ...
                throw new ValidatorError($this, 'max_size', array('max_size' => round($max / 1024, 0), 'size' => (int) $value['size']));
            case UPLOAD_ERR_FORM_SIZE:
                throw new ValidatorError($this, 'max_size', array('max_size' => 0, 'size' => (int) $value['size']));
            case UPLOAD_ERR_PARTIAL:
                throw new ValidatorError($this, 'partial');
            case UPLOAD_ERR_NO_TMP_DIR:
                throw new ValidatorError($this, 'no_tmp_dir');
            case UPLOAD_ERR_CANT_WRITE:
                throw new ValidatorError($this, 'cant_write');
            case UPLOAD_ERR_EXTENSION:
                throw new ValidatorError($this, 'extension');
        }
        // check file size
        if ($this->hasOption('max_size') && $this->getOption('max_size') < (int) $value['size']) {
            throw new ValidatorError($this, 'max_size', array('max_size' => round($this->getOption('max_size') / 1024, 0), 'size' => (int) $value['size']));
        }
        ...
        if ($this->hasOption('mime_types')) {
            ...
            if (!in_array($mimeType, array_map('strtolower', $mimeTypes))) {
                throw new ValidatorError($this, 'mime_types', array('mime_types' => $mimeTypes, 'mime_type' => $mimeType));
            }
        }
...
class_alias(ValidatorFile::class, 'sfValidatorFile', false);