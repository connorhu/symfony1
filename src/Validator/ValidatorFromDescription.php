 ...
 * @version SVN: $Id$
 */
class ValidatorFromDescription extends ValidatorDecorator
{
    ...
    protected function tokenize($string)
    {
        ...
        while ($i < $len) {
            if (preg_match('/^([a-z0-9_\\-]+)\\s*(<=|>=|<|>|==|!=)/i', substr($string, $i), $match)) {
                ...
                $i += strlen($match[0]);
                $rightField = $match[1];
                $tokens[] = new ValidatorFDToken('sfValidatorSchemaCompare', array($leftField, $operator, $rightField, $arguments[0], isset($arguments[1]) ? $arguments[1] : array()));
            } elseif (preg_match('/^(and|or)/i', substr($string, $i), $match)) {
                // all, any validador
                $i += strlen($match[0]);
                // arguments (optional)
                $arguments = $this->parseArguments($string, $i);
                $tokens[] = new ValidatorFDTokenOperator(strtolower($match[1]), $arguments);
            } elseif (preg_match('/^(?:([a-z0-9_\\-]+)\\:)?([a-z0-9_\\-]+)/i', substr($string, $i), $match)) {
                // single validator (optionally filtered)
                $i += strlen($match[0]);
                $class = 'sfValidator' . $match[2];
                $arguments = $this->parseArguments($string, $i);
                $token = new ValidatorFDToken($class, array($arguments[0], isset($arguments[1]) ? $arguments[1] : array()));
                if ($match[1]) {
                    $token = new ValidatorFDTokenFilter($match[1], $token);
                }
                $tokens[] = $token;
            } elseif ('(' == $string[$i]) {
                $tokens[] = new ValidatorFDTokenLeftBracket();
                ++$i;
            } elseif (')' == $string[$i]) {
                $tokens[] = new ValidatorFDTokenRightBracket();
                ++$i;
    ...
    protected function parseArguments($string, &$i)
    {
        ...
        ++$i;
        return YamlInline::load('[' . (!$args ? '{}' : $args) . ']');
    }
    /**
    ...
    protected function convertInfixToRpn($tokens)
    {
        ...
        foreach ($tokens as $token) {
            switch (get_class($token)) {
                ...
                case 'sfValidatorFDTokenRightBracket':
                    while (!$operatorStack[count($operatorStack) - 1] instanceof ValidatorFDTokenLeftBracket) {
        ...
        while (count($operatorStack)) {
            ...
            if ($token instanceof ValidatorFDTokenLeftBracket || $token instanceof ValidatorFDTokenRightBracket) {
    ...
    protected function reduceTokens($tokens, $method)
    {
        ...
        while (count($tokens) > 1) {
            $i = 0;
            while (isset($tokens[$i]) && !$tokens[$i] instanceof ValidatorFDTokenOperator) {
                ++$i;
            }
...
class_alias(ValidatorFromDescription::class, 'sfValidatorFromDescription', false);