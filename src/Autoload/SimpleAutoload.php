...
use Symfony1\Components\Config\AutoloadConfigHandler;
...
class SimpleAutoload
{
     ...
     *
     * @return SimpleAutoload a sfSimpleAutoload implementation instance
     */
    public static function getInstance($cacheFile = null)
    {
        if (!isset(self::$instance)) {
            self::$instance = new SimpleAutoload($cacheFile);
        }
    ...
    {
        $finder = Finder::type('file')->follow_link()->name('*' . $ext);
        if ($dirs = glob($dir)) {
            foreach ($dirs as $dir) {
    ...
    {
        $config = new AutoloadConfigHandler();
        foreach ($config->evaluate($files) as $class => $file) {
            $this->setClassPath($class, $file);
        }
    }
}
class_alias(SimpleAutoload::class, 'sfSimpleAutoload', false);