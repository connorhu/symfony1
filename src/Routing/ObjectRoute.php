 ...
 * @version SVN: $Id$
 */
class ObjectRoute extends RequestRoute
{
    /**
     * @var (bool | object)
     */
    ...
    public function getObject()
    {
        ...
        if (!($this->object = $this->getObjectForParameters($this->parameters)) && (!isset($this->options['allow_empty']) || !$this->options['allow_empty'])) {
            throw new Error404Exception(sprintf('Unable to find the %s object with the following parameters "%s").', $this->options['model'], str_replace("\n", '', var_export($this->filterParameters($this->parameters), true))));
        }
        return $this->object;
    }
    /**
    ...
    public function getObjects()
    {
        ...
        if (!count($this->objects) && isset($this->options['allow_empty']) && !$this->options['allow_empty']) {
            throw new Error404Exception(sprintf('No %s object found for the following parameters "%s").', $this->options['model'], str_replace("\n", '', var_export($this->filterParameters($this->parameters), true))));
        }
        return $this->objects;
    }
...
class_alias(ObjectRoute::class, 'sfObjectRoute', false);