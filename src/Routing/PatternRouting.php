 ...
 * @version SVN: $Id$
 */
class PatternRouting extends Routing
{
    /**
     * @var (string | null)
     */
    ...
    * @see sfRouting
    */
    public function initialize(EventDispatcher $dispatcher, Cache $cache = null, $options = array())
    {
    ...
    public function clearRoutes()
    {
        if ($this->options['logging']) {
            $this->dispatcher->notify(new Event($this, 'application.log', array('Clear all current routes')));
        }
        $this->routes = array();
    }
    /**
    ...
    public function insertRouteBefore($pivot, $name, $route)
    {
        if (!isset($this->routes[$pivot])) {
            throw new ConfigurationException(sprintf('Unable to insert route "%s" before inexistent route "%s".', $name, $pivot));
        }
        $routes = $this->routes;
        $this->routes = array();
        $newroutes = array();
    ...
    *
    * @param string $name The route name
    * @param Route $route A sfRoute instance
    *
    ...
    */
    public function connect($name, $route)
    {
        ...
        foreach (self::flattenRoutes($routes) as $name => $route) {
            ...
            if ($this->options['logging']) {
                $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Connect %s "%s" (%s)', get_class($route), $name, $route->getPattern()))));
            }
        }
    }
    ...
    public function generate($name, $params = array(), $absolute = false)
    {
        ...
        if ($name) {
            ...
            if (false === ($route = $this->getRouteThatMatchesParameters($params))) {
                throw new ConfigurationException(sprintf('Unable to find a matching route to generate url for params "%s".', is_object($params) ? 'Object(' . get_class($params) . ')' : str_replace("\n", '', var_export($params, true))));
            }
        }
    ...
    public function parse($url)
    {
        ...
        if ($this->options['logging']) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Match route "%s" (%s) for %s with parameters %s', $info['name'], $info['pattern'], $url, str_replace("\n", '', var_export($info['parameters'], true))))));
        }
    ...
    */
    ...
    {
        return Context::getInstance()->getConfigCache()->checkConfig('config/routing.yml', true);
    }
...
class_alias(PatternRouting::class, 'sfPatternRouting', false);