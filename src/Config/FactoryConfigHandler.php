...
* @version SVN: $Id$
*/
class FactoryConfigHandler extends YamlConfigHandler
{
    /**
    ...
    public function execute($configFiles)
    {
        ...
        foreach ($factories as $factory) {
            ...
            if (!isset($keys['class'])) {
                // missing class key
                throw new ParseException(sprintf('Configuration file "%s" specifies category "%s" with missing class key.', $configFiles[0], $factory));
            }
            $class = $keys['class'];
            if (isset($keys['file'])) {
                ...
                if (!is_readable($keys['file'])) {
                    ...
                    throw new ParseException(sprintf('Configuration file "%s" specifies class "%s" with nonexistent or unreadable file "%s".', $configFiles[0], $class, $keys['file']));
                }
                // append our data
            ...
            switch ($factory) {
                case 'controller':
                    ...
                    break;
                case 'request':
                    $parameters['no_script_name'] = Config::get('sf_no_script_name');
                ...
                case 'logger':
                    ...
                    if (isset($parameters['loggers'])) {
                        foreach ($parameters['loggers'] as $name => $keys) {
                            ...
                            if (!isset($keys['class'])) {
                                // missing class key
                                throw new ParseException(sprintf('Configuration file "%s" specifies logger "%s" with missing class key.', $configFiles[0], $name));
                            }
                            $condition = true;
...
class_alias(FactoryConfigHandler::class, 'sfFactoryConfigHandler', false);