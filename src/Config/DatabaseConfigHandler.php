...
* @version SVN: $Id$
*/
class DatabaseConfigHandler extends YamlConfigHandler
{
    /**
    ...
    protected function parse($configFiles)
    {
        ...
        foreach ($config as $name => $dbConfig) {
            ...
            if (in_array($name, $databases)) {
                ...
                throw new ParseException(sprintf('Configuration file "%s" specifies previously registered category "%s".', $configFiles[0], $name));
            }
            // add this database
            $databases[] = $name;
            ...
            if (!isset($dbConfig['class'])) {
                // missing class key
                throw new ParseException(sprintf('Configuration file "%s" specifies category "%s" with missing class key.', $configFiles[0], $name));
            }
            if (isset($dbConfig['file'])) {
                // we have a file to include
                if (!is_readable($dbConfig['file'])) {
                    // database file doesn't exist
                    throw new ParseException(sprintf('Configuration file "%s" specifies class "%s" with nonexistent or unreadable file "%s".', $configFiles[0], $dbConfig['class'], $dbConfig['file']));
                }
                // append our data
...
class_alias(DatabaseConfigHandler::class, 'sfDatabaseConfigHandler', false);