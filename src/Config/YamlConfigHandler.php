...
abstract class YamlConfigHandler extends ConfigHandler
{
    ...
    public static function parseYamls($configFiles)
    {
        ...
        foreach ($configFiles as $configFile) {
            ...
            $config = Toolkit::arrayDeepMerge($config, $values);
        }
        return $config;
    }
    /**
    ...
    public static function parseYaml($configFile)
    {
        if (!is_readable($configFile)) {
            ...
            throw new ConfigurationException(sprintf('Configuration file "%s" does not exist or is not readable.', $configFile));
        }
        // parse our config
        $config = Yaml::load($configFile, Config::get('sf_charset', 'UTF-8'));
        if (false === $config) {
            // configuration couldn't be parsed
            throw new ParseException(sprintf('Configuration file "%s" could not be parsed', $configFile));
        }
        return null === $config ? array() : $config;
    }
    public static function flattenConfiguration($config)
    {
        $config['all'] = Toolkit::arrayDeepMerge(isset($config['default']) && is_array($config['default']) ? $config['default'] : array(), isset($config['all']) && is_array($config['all']) ? $config['all'] : array());
    ...
    {
        return Toolkit::arrayDeepMerge(isset($config['default']) && is_array($config['default']) ? $config['default'] : array(), isset($config['all']) && is_array($config['all']) ? $config['all'] : array(), isset($config[Config::get('sf_environment')]) && is_array($config[Config::get('sf_environment')]) ? $config[Config::get('sf_environment')] : array());
    }
    /**
...
class_alias(YamlConfigHandler::class, 'sfYamlConfigHandler', false);