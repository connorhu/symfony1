 ...
 * @version SVN: $Id$
 */
class FilterConfigHandler extends YamlConfigHandler
{
    /**
    ...
    public function execute($configFiles)
    {
        ...
        foreach ($config as $category => $keys) {
            ...
            if (!isset($keys['class'])) {
                // missing class key
                throw new ParseException(sprintf('Configuration file "%s" specifies category "%s" with missing class key.', $configFiles[0], $category));
            }
            $class = $keys['class'];
            if (isset($keys['file'])) {
        ...
        if (!$rendering) {
            throw new ParseException(sprintf('Configuration file "%s" must register a filter of type "rendering".', $configFiles[0]));
        }
        if (!$execution) {
            throw new ParseException(sprintf('Configuration file "%s" must register a filter of type "execution".', $configFiles[0]));
        }
        // compile data
    ...
    public static function getConfiguration(array $configFiles)
    {
        ...
        foreach (array_slice($configFiles, 1) as $i => $configFile) {
            ...
            foreach (static::parseYaml($configFile) as $key => $value) {
                $value = (array) $value;
                $config[$key] = isset($previous[$key]) ? Toolkit::arrayDeepMerge($previous[$key], $value) : $value;
            }
            ...
            foreach (array_keys($previous) as $key) {
                if (!isset($config[$key])) {
                    throw new ConfigurationException(sprintf('The filter name "%s" is defined in "%s" but not present in "%s" file. To disable a filter, add a "enabled" key with a false value.', $key, $configFiles[$i], $configFile));
                }
            }
        }
...
class_alias(FilterConfigHandler::class, 'sfFilterConfigHandler', false);