    ...
    */
    public function checkConfig($configPath, $optional = false)
    {
        if (Config::get('sf_debug') && Config::get('sf_logging_enabled')) {
            $timer = TimerManager::getTimer('Configuration');
        }
        // the cache filename we'll be using
        $cache = $this->getCacheName($configPath);
        if (!Config::get('sf_debug') && !Config::get('sf_test') && is_readable($cache)) {
            return $cache;
        }
        if (!Toolkit::isPathAbsolute($configPath)) {
        ...
        if (!isset($files[0])) {
            ...
            throw new ConfigurationException(sprintf('Configuration "%s" does not exist or is unreadable.', $configPath));
        }
    ...
    {
        Toolkit::clearDirectory(Config::get('sf_config_cache_dir'));
    }
    /**
    ...
    public function getCacheName($config)
    {
        ...
        $config .= '.php';
        return Config::get('sf_config_cache_dir') . '/' . $config;
    }
    /**
    ...
    protected function callHandler($handler, $configs, $cache)
    {
        ...
        if (!$handlerInstance) {
            ...
            throw new ConfigurationException(sprintf('Configuration file "%s" does not have a registered handler.', implode(', ', $configs)));
        }
     ...
     *
     * @return ConfigHandler A sfConfigHandler instance
     */
    ...
    protected function loadConfigHandlers()
    {
        ...
        $this->handlers['config_handlers.yml'] = new RootConfigHandler();
        ...
        if (!is_readable($sf_app_module_dir = Config::get('sf_app_module_dir'))) {
            return;
        }
        // ignore names
    ...
    protected function writeCacheFile($config, $cache, $data)
    {
        ...
        if (!is_dir($cacheDir) && !@mkdir($cacheDir, 0777, true) && !is_dir($cacheDir)) {
            throw new CacheException(sprintf('Failed to make cache directory "%s" while generating cache for configuration file "%s".', $cacheDir, $config));
        }
        $tmpFile = tempnam($cacheDir, basename($cache));
        if (!($fp = @fopen($tmpFile, 'wb'))) {
            throw new CacheException(sprintf('Failed to write cache file "%s" generated from configuration file "%s".', $tmpFile, $config));
        }
        @fwrite($fp, $data);
        @fclose($fp);
...
class_alias(ConfigCache::class, 'sfConfigCache', false);