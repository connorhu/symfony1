 ...
 * @version SVN: $Id$
 */
class ViewConfigHandler extends YamlConfigHandler
{
    /**
...
EOF;
    ...
    protected function addHtmlHead($viewName = '')
    {
        ...
        foreach ($this->mergeConfigValue('metas', $viewName) as $name => $content) {
            $data[] = sprintf("  \$response->addMeta('%s', '%s', false, false);", $name, str_replace('\'', '\\\'', preg_replace('/&amp;(?=\\w+;)/', '&', htmlspecialchars((string) $content, ENT_QUOTES, Config::get('sf_charset')))));
        }
        return implode("\n", $data) . "\n";
    }
    /**
...
class_alias(ViewConfigHandler::class, 'sfViewConfigHandler', false);