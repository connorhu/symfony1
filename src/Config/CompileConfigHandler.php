...
* @version SVN: $Id$
*/
class CompileConfigHandler extends YamlConfigHandler
{
    /**
    ...
    public function execute($configFiles)
    {
        ...
        foreach ($config as $file) {
            if (!is_readable($file)) {
                // file doesn't exist
                throw new ParseException(sprintf('Configuration file "%s" specifies nonexistent or unreadable file "%s".', $configFiles[0], $file));
            }
            $contents = file_get_contents($file);
            // strip comments (not in debug mode)
            if (!Config::get('sf_debug')) {
                $contents = Toolkit::stripComments($contents);
            }
            // strip php tags
            $contents = Toolkit::pregtr($contents, array('/^\\s*<\\?(php\\s*)?/m' => '', '/^\\s*\\?>/m' => ''));
...
class_alias(CompileConfigHandler::class, 'sfCompileConfigHandler', false);