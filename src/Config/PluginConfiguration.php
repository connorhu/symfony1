...
use Symfony1\Components\Task\Test\TestAllTask;
use Symfony1\Components\Task\Test\TestFunctionalTask;
use Symfony1\Components\Task\Test\TestUnitTask;
...
abstract class PluginConfiguration
{
     ...
     */
    public function __construct(ProjectConfiguration $configuration, $rootDir = null, $name = null)
    {
    ...
    */
    ...
    public function filterAutoloadConfig(Event $event, array $config)
    {
        ...
        if (!isset($config['autoload'][$this->name . '_lib'])) {
            $config['autoload'] = array_merge(array($this->name . '_lib' => array('path' => $this->rootDir . '/lib', 'recursive' => true)), $config['autoload']);
        }
        if (!isset($config['autoload'][$this->name . '_module_libs'])) {
            $config['autoload'] = array_merge(array($this->name . '_module_libs' => array('path' => $this->rootDir . '/modules/*/lib', 'recursive' => true, 'prefix' => 1)), $config['autoload']);
        }
        return $config;
    }
    /**
     ...
     */
    public function filterTestFiles(Event $event, $files)
    {
        $task = $event->getSubject();
        if ($task instanceof TestAllTask) {
            $directory = $this->rootDir . '/test';
            $names = array();
        } elseif ($task instanceof TestFunctionalTask) {
            $directory = $this->rootDir . '/test/functional';
            $names = $event['arguments']['controller'];
        } elseif ($task instanceof TestUnitTask) {
...
class_alias(PluginConfiguration::class, 'sfPluginConfiguration', false);