...
abstract class WebController extends Controller
{
    ...
    public function convertUrlStringToParameters($url)
    {
      ...
      /x', $queryString, $matches, PREG_SET_ORDER | PREG_OFFSET_CAPTURE);
            ...
            if (!$matched) {
                throw new ParseException(sprintf('Unable to parse query string "%s".', $queryString));
            }
        }
    ...
    */
    public function redirect($url, $delay = 0, $statusCode = 302)
    {
        ...
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Redirect to "%s"', $url))));
        }
        // redirect
        /**
        ...
        $response->setContent(sprintf('<html><head><meta http-equiv="refresh" content="%d;url=%s"/></head></html>', $delay, htmlspecialchars($url, ENT_QUOTES, Config::get('sf_charset'))));
        $response->send();
    }
}
class_alias(WebController::class, 'sfWebController', false);