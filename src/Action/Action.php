...
use Symfony1\Components\Controller\WebController;
...
use Symfony1\Components\Exception\StopException;
...
use function get_partial;
use function get_component;
 ...
 * @version SVN: $Id$
 *
 * @method WebController getController()
 * @method WebResponse getResponse()
 */
abstract class Action extends Component
{
    ...
    {
        throw new Error404Exception($this->get404Message($message));
    }
    /**
    ...
    {
        return $this->redirect('/' . Config::get('sf_error_404_module') . '/' . Config::get('sf_error_404_action'));
    }
    /**
    ...
    public function forward($module, $action)
    {
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Forward to action "%s/%s"', $module, $action))));
        }
    ...
    */
    ...
    {
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Change template to "%s/%s"', null === $module ? 'CURRENT' : $module, $name))));
        }
        if (null !== $module) {
            $dir = $this->context->getConfiguration()->getTemplateDir($module, $name . View::SUCCESS . '.php');
            $name = $dir . '/' . $name;
        }
        Config::set('symfony.view.' . $this->getModuleName() . '_' . $this->getActionName() . '_template', $name);
    }
    /**
    ...
    */
    ...
    {
        return Config::get('symfony.view.' . $this->getModuleName() . '_' . $this->getActionName() . '_template');
    }
    /**
    ...
    {
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Change layout to "%s"', $name))));
        }
        Config::set('symfony.view.' . $this->getModuleName() . '_' . $this->getActionName() . '_layout', $name);
    }
    /**
    ...
    */
    ...
    {
        return Config::get('symfony.view.' . $this->getModuleName() . '_' . $this->getActionName() . '_layout');
    }
    /**
    ...
    {
        Config::set('mod_' . strtolower($this->getModuleName()) . '_view_class', $class);
    }
    /**
...
class_alias(Action::class, 'sfAction', false);