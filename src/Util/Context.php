...
use Symfony1\Components\Exception\FactoryException;
...
use Symfony1\Components\Controller\FrontWebController;
use Symfony1\Components\Mailer\Mailer;
use Symfony1\Components\Log\Logger;
use Symfony1\Components\Log\NoLogger;
...
use Symfony1\Components\Storage\Storage;
use Symfony1\Components\View\ViewCacheManager;
...
use Symfony1\Components\Command\Formatter;
use Symfony1\Components\Config\ConfigCache;
...
class Context implements ArrayAccess
{
    ...
    */
    ...
    {
        $event = $this->dispatcher->notifyUntil(new Event($this, 'context.method_not_found', array('method' => $method, 'arguments' => $arguments)));
     ...
     *
     * @param ApplicationConfiguration $configuration An sfApplicationConfiguration instance
     ...
     *
     * @return Context An sfContext instance
     *
     * @throws FactoryException
     */
    public static function createInstance(ApplicationConfiguration $configuration, $name = null, $class = __CLASS__)
    {
        if (null === $name) {
        ...
        if (!self::$instances[$name] instanceof Context) {
            throw new FactoryException(sprintf('Class "%s" is not of the type sfContext.', $class));
        }
     ...
     * @param ApplicationConfiguration $configuration An sfApplicationConfiguration instance
     */
    public function initialize(ApplicationConfiguration $configuration)
    {
     ...
     * @param string $class The context class to use (sfContext by default)
     *
     * @return Context an sfContext implementation instance
     *
     * @throws Exception
     */
    ...
    {
        if (Config::get('sf_use_database')) {
            // setup our database connections
            $this->factories['databaseManager'] = new DatabaseManager($this->configuration, array('auto_shutdown' => false));
        }
        // create a new action stack
        $this->factories['actionStack'] = new ActionStack();
        if (Config::get('sf_debug') && Config::get('sf_logging_enabled')) {
            $timer = TimerManager::getTimer('Factories');
        }
        // include the factories configuration
        require $this->configuration->getConfigCache()->checkConfig('config/factories.yml');
        $this->dispatcher->notify(new Event($this, 'context.load_factories'));
        if (Config::get('sf_debug') && Config::get('sf_logging_enabled')) {
            // @var $timer sfTimer
            $timer->addTime();
        }
    ...
    {
        if (!isset(self::$instances[$name])) {
            $currentConfiguration = Context::getInstance()->getConfiguration();
            Context::createInstance(ProjectConfiguration::getApplicationConfiguration($name, $currentConfiguration->getEnvironment(), $currentConfiguration->isDebug()));
        }
        self::$current = $name;
        Context::getInstance()->getConfiguration()->activate();
    }
    /**
     * Returns the configuration instance.
     *
     * @return ApplicationConfiguration The current application configuration instance
     */
     ...
     *
     * @return EventDispatcher An sfEventDispatcher instance
     */
    ...
    */
     ...
     *
     * @return ActionStack the sfActionStack instance
     */
     ...
     *
     * @return FrontWebController the current sfController implementation instance
     */
     ...
     *
     * @return Mailer the current sfMailer implementation instance
     */
     ...
     * Retrieve the logger.
     *
     * @return Logger the current sfLogger implementation instance
     */
    public function getLogger()
    {
        if (!isset($this->factories['logger'])) {
            $this->factories['logger'] = new NoLogger($this->dispatcher);
        }
    ...
    */
     ...
     *
     * @return DatabaseManager the current sfDatabaseManager instance
     */
    ...
    */
    public function getModuleDirectory()
    {
        ...
        if (isset($this->factories['actionStack']) && ($lastEntry = $this->factories['actionStack']->getLastEntry())) {
            // @var $lastEntry sfActionStackEntry
            return Config::get('sf_app_module_dir') . '/' . $lastEntry->getModuleName();
        }
    }
    /**
    ...
    */
     ...
     *
     * @param Response $response an sfResponse instance
     */
     ...
     *
     * @return Storage the current sfStorage implementation instance
     */
     ...
     *
     * @return ViewCacheManager the current sfViewCacheManager implementation instance
     */
     ...
     *
     * @return I18N the current sfI18N implementation instance
     *
     * @throws ConfigurationException
     */
    public function getI18N()
    {
        if (!Config::get('sf_i18n')) {
            throw new ConfigurationException('You must enable i18n support in your settings.yml configuration file.');
        }
     ...
     *
     * @return Routing the current sfRouting implementation instance
     */
     ...
     *
     * @return ServiceContainer the current sfServiceContainer implementation instance
     */
    public function getServiceContainer()
    {
        if (!isset($this->factories['serviceContainer'])) {
            ...
            $this->factories['serviceContainer']->setService('sf_formatter', new Formatter());
     ...
     */
    public function filterTemplateParameters(Event $event, $parameters)
    {
...
class_alias(Context::class, 'sfContext', false);