...
abstract class BrowserBase
{
     ...
     *
     * @return BrowserBase This sfBrowserBase instance
     */
    public function setCookie($name, $value, $expire = null, $path = '/', $domain = '', $secure = false, $httpOnly = false)
    {
        $this->cookieJar[$name] = array('name' => $name, 'value' => $value, 'expire' => $expire, 'path' => $path, 'domain' => $domain, 'secure' => (bool) $secure, 'httpOnly' => $httpOnly);
        return $this;
    }
    /**
    ...
    public function resetCurrentException()
    {
        ...
        ExceptionException::clearLastException();
    }
    /**
    ...
    {
        return null === $this->getCurrentException() || $this->getCurrentException() instanceof Error404Exception;
    }
    /**
     * Follow redirects?
     *
     * @return BrowserBase
     *
     * @throws ExceptionException If request was not a redirect
     */
    ...
    */
    ...
    public function doClickElement(DOMElement $item, $arguments = array(), $options = array())
    {
        ...
        $defaults = array();
        $arguments = Toolkit::arrayDeepMerge($this->fields, $arguments);
        $xpath = $this->getResponseDomXpath();
        foreach ($xpath->query('descendant::input | descendant::textarea | descendant::select', $item) as $element) {
            ...
            if ('input' == $nodeName && ('checkbox' == $element->getAttribute('type') || 'radio' == $element->getAttribute('type'))) {
                ...
                $filename = array_key_exists($elementName, $arguments) ? $arguments[$elementName] : Toolkit::getArrayValueForPath($arguments, $elementName, '');
        ...
        $arguments = Toolkit::arrayDeepMerge($defaults, $arguments);
        if (in_array($method, array('post', 'put', 'delete'))) {
     ...
     * Calls a request to a uri.
     */
    protected abstract function doCall();
    /**
...
class_alias(BrowserBase::class, 'sfBrowserBase', false);