...
class Finder
{
     ...
     * @param int $level
     *
     * @return Finder current sfFinder object
     */
     ...
     *
     * @return Finder new sfFinder object
     */
    ...
    public function exec()
    {
        ...
        for ($i = 0; $i < $numargs; ++$i) {
            if (is_array($args[$i]) && !method_exists($args[$i][0], $args[$i][1])) {
                throw new Exception(sprintf('method "%s" does not exist for object "%s".', $args[$i][1], $args[$i][0]));
            }
            if (!is_array($args[$i]) && !function_exists($args[$i])) {
                throw new Exception(sprintf('function "%s" does not exist.', $args[$i]));
            }
            $this->execs[] = $args[$i];
        }
        return $this;
...
class_alias(Finder::class, 'sfFinder', false);