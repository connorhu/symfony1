...
* @version SVN: $Id$
*/
class DomCssSelector implements Countable, Iterator
{
    public $nodes = array();
    private $count;
    ...
    public function matchSingle($selector)
    {
        ...
        return $nodes ? new DomCssSelector($nodes[0]) : new DomCssSelector(array());
    }
    public function matchAll($selector)
    {
        $nodes = $this->getElements($selector);
        return $nodes ? new DomCssSelector($nodes) : new DomCssSelector(array());
    }
    /**
...
class_alias(DomCssSelector::class, 'sfDomCssSelector', false);