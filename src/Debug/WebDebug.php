...
use Symfony1\Components\Log\VarLogger;
...
class WebDebug
{
    ...
    * @param VarLogger $logger The logger
    * @param array $options An array of options
    */
    public function __construct(EventDispatcher $dispatcher, VarLogger $logger, array $options = array())
    {
        ...
        $this->configure();
        $this->dispatcher->notify(new Event($this, 'debug.web.load_panels'));
    }
    /**
    ...
    {
        $this->setPanel('symfony_version', new WebDebugPanelSymfonyVersion($this));
        if (Config::get('sf_debug') && Config::get('sf_cache')) {
            $this->setPanel('cache', new WebDebugPanelCache($this));
        }
        if (Config::get('sf_logging_enabled')) {
            $this->setPanel('config', new WebDebugPanelConfig($this));
            $this->setPanel('view', new WebDebugPanelView($this));
        }
        $this->setPanel('logs', new WebDebugPanelLogs($this));
        $this->setPanel('memory', new WebDebugPanelMemory($this));
        if (Config::get('sf_debug')) {
            $this->setPanel('time', new WebDebugPanelTimer($this));
        }
        $this->setPanel('mailer', new WebDebugPanelMailer($this));
    }
    /**
     * Gets the logger.
     *
     * @return VarLogger The logger instance
     */
     ...
     * @param WebDebugPanel $panel The panel
     */
    public function setPanel($name, WebDebugPanel $panel)
    {
        ...
        foreach ($this->panels as $name => $panel) {
            if ($title = $panel->getTitle()) {
                if (($content = $panel->getPanelContent()) || $panel->getTitleUrl()) {
                    ...
                    $titles[] = sprintf('<li%s><a title="%s" href="%s"%s>%s</a></li>', $panel->getStatus() ? ' class="sfWebDebug' . ucfirst($this->getPriority($panel->getStatus())) . '"' : '', $panel->getPanelTitle(), $panel->getTitleUrl() ?: '#', $panel->getTitleUrl() ? '' : ' onclick="sfWebDebugShowDetailsFor(\'' . $id . '\'); return false;"', $title);
                    $panels[] = sprintf('<div id="%s" class="sfWebDebugTop" style="display:%s"><h1>%s</h1>%s</div>', $id, $name == $current ? 'block' : 'none', $panel->getPanelTitle(), $content);
                } else {
...
class_alias(WebDebug::class, 'sfWebDebug', false);