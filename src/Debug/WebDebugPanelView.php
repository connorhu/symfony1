...
use Symfony1\Components\Form\FormFieldSchema;
use function count;
 ...
 * @version SVN: $Id$
 */
class WebDebugPanelView extends WebDebugPanel
{
     ...
     */
    public function listenForChangeAction(Event $event)
    {
        $this->actions = array();
        $this->partials = array();
    }
    ...
    protected function getLastTemplate($class = 'sfPHPView')
    {
        foreach (array_reverse($this->webDebug->getLogger()->getLogs()) as $log) {
            if (($class == $log['type'] || class_exists($log['type'], false) && is_subclass_of($log['type'], $class)) && preg_match('/^Render "(.*)"$/', $log['message'], $match)) {
                return $match[1];
            }
        }
    }
    ...
    protected function renderTemplateInformation($file, $parameters, $label = 'Template')
    {
        ...
        if (count($parameters)) {
            ...
            foreach ($parameters as $name => $parameter) {
                ...
                $html[] = $this->webDebug->getEventDispatcher()->filter(new Event($this, 'debug.web.view.filter_parameter_html', array('parameter' => $parameter)), $presentation)->getReturnValue();
            }
            $html[] = '</ul>';
        } else {
     ...
     * @return string
     */
    protected function formatFormAsHtml($name, Form $form)
    {
        static $i = 0;
        ++$i;
        if ($form->hasErrors() && Logger::NOTICE < $this->getStatus()) {
            $this->setStatus(Logger::NOTICE);
        }
        $html = array();
     ...
     * @return string
     */
    protected function formatFormFieldSchemaAsHtml(FormFieldSchema $fieldSchema, $nameFormat = '%s')
    {
        $html = array();
        foreach ($fieldSchema as $field) {
            $name = sprintf($nameFormat, $this->varExport($field->getName()));
            if ($field instanceof FormFieldSchema) {
    ...
    protected function shortenTemplatePath($path)
    {
        ...
        return str_replace('SF_ROOT_DIR' . DIRECTORY_SEPARATOR, '', Debug::shortenFilePath($path));
    }
    /**
...
class_alias(WebDebugPanelView::class, 'sfWebDebugPanelView', false);