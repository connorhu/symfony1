 ...
 * @version SVN: $Id$
 */
class WebDebugPanelLogs extends WebDebugPanel
{
    ...
    {
        $event = $this->webDebug->getEventDispatcher()->filter(new Event($this, 'debug.web.filter_logs'), $this->webDebug->getLogger()->getLogs());
      ...
      <tr>
        ...
        foreach ($logs as $log) {
            ...
            ++$line_nb;
            $html .= sprintf("<tr class='sfWebDebugLogLine sfWebDebug%s %s'><td class=\"sfWebDebugLogNumber\">%s</td><td class=\"sfWebDebugLogType\">%s&nbsp;%s</td><td>%s %s</td></tr>\n", ucfirst($priority), $log['type'], $line_nb, '<img src="' . $this->webDebug->getOption('image_root_path') . '/' . $priority . '.png" alt="' . ucfirst($priority) . '"/>', class_exists($log['type'], false) ? $this->formatFileLink($log['type']) : $log['type'], $this->formatLogLine($log['message']), $this->getToggleableDebugStack($log['debug_backtrace']));
        }
        $html .= '</table>';
        $types = array();
    ...
    protected function formatLogLine($logLine)
    {
        ...
        if (!$constants) {
            foreach (array('sf_app_dir', 'sf_root_dir', 'sf_symfony_lib_dir') as $constant) {
                $constants[realpath(Config::get($constant)) . DIRECTORY_SEPARATOR] = $constant . DIRECTORY_SEPARATOR;
            }
        }
        // escape HTML
        $logLine = htmlspecialchars($logLine, ENT_QUOTES, Config::get('sf_charset'));
        // replace constants value with constant name
        $logLine = str_replace(array_keys($constants), array_values($constants), $logLine);
        $logLine = Toolkit::pregtr($logLine, array('/&quot;(.+?)&quot;/s' => '"<span class="sfWebDebugLogInfo">\\1</span>"', '/^(.+?)\\(\\)\\:/S' => '<span class="sfWebDebugLogInfo">\\1()</span>:', '/line (\\d+)$/' => 'line <span class="sfWebDebugLogInfo">\\1</span>'));
...
class_alias(WebDebugPanelLogs::class, 'sfWebDebugPanelLogs', false);