...
use Symfony1\Components\User\BasicSecurityUser;
...
class Debug
{
    ...
    {
        return array('version' => SYMFONY_VERSION, 'path' => Config::get('sf_symfony_lib_dir'));
    }
    /**
    ...
    {
        $values = array('php' => phpversion(), 'os' => php_uname(), 'extensions' => get_loaded_extensions());
     ...
     *
     * @param Request $request A sfRequest instance
     *
     * @return array The request parameter holders
     */
    public static function requestAsArray(Request $request = null)
    {
        if (!$request) {
            return array();
        }
     ...
     *
     * @param WebResponse $response A sfResponse instance
     *
     * @return array The response parameters
     */
    public static function responseAsArray(Response $response = null)
    {
        if (!$response) {
            return array();
        }
     ...
     *
     * @param User $user A sfUser instance
     *
     * @return array The user parameters
     */
    public static function userAsArray(User $user = null)
    {
        if (!$user) {
            return array();
        }
        ...
        if ($user instanceof BasicSecurityUser) {
     ...
     *
     * @param ParameterHolder $parameterHolder A sfParameterHolder instance
    ...
    public static function shortenFilePath($file)
    {
        foreach (array('sf_root_dir', 'sf_symfony_lib_dir') as $key) {
            if (0 === strpos($file, $value = Config::get($key))) {
...
class_alias(Debug::class, 'sfDebug', false);