...
namespace Symfony1\Components\I18n;
...
class I18N
{
     ...
     * @see initialize()
     */
    public function __construct(ApplicationConfiguration $configuration, Cache $cache = null, $options = array())
    {
    ...
    *
    * @param ApplicationConfiguration $configuration A sfApplicationConfiguration instance
    * @param Cache $cache A sfCache instance
    * @param array $options An array of options
    */
    public function initialize(ApplicationConfiguration $configuration, Cache $cache = null, $options = array())
    {
     ...
     *
     * @return ApplicationConfiguration An sfApplicationConfiguration instance
     */
    ...
    public function setMessageSource($dirs, $culture = null)
    {
        if (null === $dirs) {
            ...
            $this->messageSource = MessageSource::factory('Aggregate', array_map(array($this, 'createMessageSource'), $dirs));
        }
        if (null !== $this->cache) {
     ...
     *
     * @return MessageSource A sfMessageSource object
     */
    public function createMessageSource($dir = null)
    {
        return MessageSource::factory($this->options['source'], self::isMessageSourceFileBased($this->options['source']) ? $dir : $this->options['database']);
    }
    /**
     ...
     *
     * @return MessageFormat A sfMessageFormat object
     */
    public function getMessageFormat()
    {
        if (!isset($this->messageFormat)) {
            $this->messageFormat = new MessageFormat($this->getMessageSource(), Config::get('sf_charset'));
    ...
    {
        $c = CultureInfo::getInstance(null === $culture ? $this->culture : $culture);
    ...
    {
        return CultureInfo::getInstance($culture)->getNativeName();
    }
    /**
    ...
    public function getDateForCulture($date, $culture = null)
    {
        ...
        $dateFormatInfo = @DateTimeFormatInfo::getInstance(null === $culture ? $this->culture : $culture);
    ...
    public function getTimeForCulture($time, $culture = null)
    {
        ...
        $timeFormatInfo = @DateTimeFormatInfo::getInstance($culture);
        ...
        $timePositions = array('h' => false !== strpos($timeFormat, 'H') ? strpos($timeFormat, 'H') : strpos($timeFormat, 'h'), 'm' => strpos($timeFormat, 'm'), 'a' => strpos($timeFormat, 'a'));
        asort($timePositions);
        $i = 0;
     ...
     */
    public function listenToChangeCultureEvent(Event $event)
    {
