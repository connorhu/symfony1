...
*/
class MessageSource_gettext extends MessageSource_File
{
    /**
    ...
    */
    public function save($catalogue = 'messages')
    {
        ...
        if (false == is_writable($MOFile)) {
            throw new Exception(sprintf('Unable to save to file %s, file must be writable.', $MOFile));
        }
        if (false == is_writable($POFile)) {
            throw new Exception(sprintf('Unable to save to file %s, file must be writable.', $POFile));
        }
    ...
    public function delete($message, $catalogue = 'messages')
    {
        ...
        if (false == is_writable($MOFile)) {
            throw new Exception(sprintf('Unable to modify file %s, file must be writable.', $MOFile));
        }
        if (false == is_writable($POFile)) {
            throw new Exception(sprintf('Unable to modify file %s, file must be writable.', $POFile));
        }
        $po = TGettext::factory('PO', $POFile);
        $po->load();
        $result = $po->toArray();
    ...
    public function update($text, $target, $comments, $catalogue = 'messages')
    {
        ...
        if (false == is_writable($MOFile)) {
            throw new Exception(sprintf('Unable to update file %s, file must be writable.', $MOFile));
        }
        if (false == is_writable($POFile)) {
            throw new Exception(sprintf('Unable to update file %s, file must be writable.', $POFile));
        }
        $po = TGettext::factory('PO', $POFile);
        $po->load();
        $result = $po->toArray();
    ...
    protected function createMessageTemplate($catalogue)
    {
        ...
        throw new Exception(sprintf('Unable to create file %s and %s.', $po_file, $mo_file));
    }
}
class_alias(MessageSource_gettext::class, 'sfMessageSource_gettext', false);