...
class CultureInfo
{
    ...
    */
    public function getParent()
    {
        ...
        return new CultureInfo(substr($this->culture, 0, 2));
    }
    /**
    ...
    */
    protected function setCulture($culture)
    {
        if (!empty($culture)) {
            if (!preg_match('/^[a-z]{2}(_[A-Z]{2,5}){0,2}$/', $culture)) {
                throw new Exception(sprintf('Invalid culture supplied: %s', $culture));
            }
        }
    ...
    protected function loadCultureData($culture)
    {
        ...
        foreach ($files as $file) {
            ...
            if (false == is_file($filename)) {
                throw new Exception(sprintf('Data file for "%s" was not found.', $file));
            }
...
class_alias(CultureInfo::class, 'sfCultureInfo', false);