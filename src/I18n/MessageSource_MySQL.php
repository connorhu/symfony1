...
use function mysql_fetch_array;
use function count;
use function time;
...
use function mysql_select_db;
use function mysql_result;
...
*/
class MessageSource_MySQL extends MessageSource_Database
{
    /**
    ...
    */
    ...
    protected function connect()
    {
        ...
        if (empty($conn)) {
            throw new Exception(sprintf('Error in connecting to %s.', $dsninfo));
        }
        if ($dsninfo['database']) {
            if (!@mysql_select_db($dsninfo['database'], $conn)) {
                throw new Exception(sprintf('Error in connecting database, dsn: %s.', $dsninfo));
            }
        } else {
            throw new Exception('Please provide a database for message translation.');
        }
        return $conn;
    }
    /**
...
class_alias(MessageSource_MySQL::class, 'sfMessageSource_MySQL', false);