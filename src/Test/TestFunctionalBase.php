...
use Symfony1\Components\Util\BrowserBase;
use lime_test;
 ...
 * @version SVN: $Id$
 */
abstract class TestFunctionalBase
{
     ...
     *
     * @param BrowserBase $browser A sfBrowserBase instance
     * @param lime_test $lime A lime instance
     */
    public function __construct(BrowserBase $browser, lime_test $lime = null, $testers = array())
    {
     ...
     *
     * @return Tester The current sfTester instance
     */
     ...
     *
     * @param string $name The tester name
     * @param (Tester | string) $tester A sfTester instance or a tester class name
     */
     ...
     *
     * @return TestFunctionalBase The current sfTestFunctionalBase instance
     */
    public function getAndCheck($module, $action, $url = null, $code = 200)
    {
        return $this->get(null !== $url ? $url : sprintf('/%s/%s', $module, $action))->with('request')->begin()->isParameter('module', $module)->isParameter('action', $action)->end()->with('response')->isStatusCode($code);
    }
    /**
     * Posts a uri.
     *
    ...
    public function handleException($exception)
    {
        ...
        array_unshift($traceData, array('function' => '', 'file' => null != $exception->getFile() ? $exception->getFile() : 'n/a', 'line' => null != $exception->getLine() ? $exception->getLine() : 'n/a', 'args' => array()));
        $traces = array();
        ...
        for ($i = 0, $count = count($traceData); $i < $count; ++$i) {
            ...
            $this->test()->error(sprintf($lineFormat, isset($traceData[$i]['class']) ? $traceData[$i]['class'] : '', isset($traceData[$i]['type']) ? $traceData[$i]['type'] : '', $traceData[$i]['function'], $file, $line));
        }
        $this->test()->fail('An uncaught exception has been thrown.');
    }
}
class_alias(TestFunctionalBase::class, 'sfTestFunctionalBase', false);