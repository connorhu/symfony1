 ...
 * @version SVN: $Id$
 */
class TesterForm extends Tester
{
    protected $form;
    /**
     * Constructor.
    ...
    public function initialize()
    {
        if (null === $this->form) {
            ...
            foreach ($action->getVarHolder()->getAll() as $name => $value) {
                if ($value instanceof Form && $value->isBound()) {
                    $this->form = $value;
                    break;
                }
            }
     ...
     *
     * @return Form The current sfForm form instance
     */
    ...
    public function hasErrors($value = true)
    {
        ...
        if ((false === $value || is_int($value) && $value != count($this->form->getErrorSchema())) && $this->form->hasErrors()) {
    ...
    public function isError($field, $value = true)
    {
        ...
        if (null === $field) {
            $error = new ValidatorErrorSchema(new ValidatorPass());
...
class_alias(TesterForm::class, 'sfTesterForm', false);