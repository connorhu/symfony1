...
class Exception extends Exception1
{
     ...
     * Wraps an Throwable.
     *
     * @param (Exception1 | Throwable) $e An Throwable instance
     *
     * @return Exception An sfException instance that wraps the given Throwable object
     */
    public static function createFromException($e)
    {
        $exception = new Exception(sprintf('Wrapped %s: %s', get_class($e), $e->getMessage()));
     ...
     *
     * @param (Exception1 | Throwable) $e A Throwable instance
     */
     ...
     *
     * @return (Exception1 | Throwable) An Throwable instance
     */
    ...
    */
    ...
    {
        $templatePaths = array(Config::get('sf_app_config_dir') . '/error', Config::get('sf_config_dir') . '/error', __DIR__ . '/data');
     ...
     *
     * @param (Exception1 | Throwable) $exception
     */
    protected static function outputStackTrace($exception)
    {
        ...
        $response = null;
        if (class_exists('sfContext', false) && Context::hasInstance() && is_object($request = Context::getInstance()->getRequest()) && is_object($response = Context::getInstance()->getResponse())) {
            /** @var sfWebRequest $request */
            /**
             * @var WebResponse $response
             */
            $dispatcher = Context::getInstance()->getEventDispatcher();
            if (Config::get('sf_logging_enabled')) {
                $priority = $exception instanceof Error404Exception ? Logger::ERR : Logger::CRIT;
                $dispatcher->notify(new Event($exception, 'application.log', array($exception->getMessage(), 'priority' => $priority)));
            }
            $event = $dispatcher->notifyUntil(new Event($exception, 'application.throw_exception'));
            ...
            if (!Config::get('sf_test')) {
                header('Content-Type: text/html; charset=' . Config::get('sf_charset', 'utf-8'));
            }
        }
        ...
        if (class_exists('sfContext', false) && Context::hasInstance()) {
            $context = Context::getInstance();
            $settingsTable = self::formatArrayAsHtml(Debug::settingsAsArray());
            $requestTable = self::formatArrayAsHtml(Debug::requestAsArray($context->getRequest()));
            $responseTable = self::formatArrayAsHtml(Debug::responseAsArray($context->getResponse()));
            $userTable = self::formatArrayAsHtml(Debug::userAsArray($context->getUser()));
            $globalsTable = self::formatArrayAsHtml(Debug::globalsAsArray());
        }
        ...
        if ($template = self::getTemplatePathForError($format, true)) {
            if (isset($dispatcher)) {
                ...
                $event = $dispatcher->filter(new Event($response, 'response.filter_content'), $content);
     ...
     *
     * @param (Exception1 | Throwable) $exception An Throwable implementation instance
    ...
    protected static function getTraces($exception, $format = 'txt')
    {
        ...
        array_unshift($traceData, array('function' => '', 'file' => null != $exception->getFile() ? $exception->getFile() : null, 'line' => null != $exception->getLine() ? $exception->getLine() : null, 'args' => array()));
        $traces = array();
        if ('html' == $format) {
    ...
    protected static function formatFile($file, $line, $format = 'html', $text = null)
    {
        ...
        if ('html' == $format && $file && $line && ($linkFormat = Config::get('sf_file_link_format', ini_get('xdebug.file_link_format')))) {
    ...
    protected static function escape($value)
    {
        ...
        return htmlspecialchars($value, ENT_QUOTES, Config::get('sf_charset', 'UTF-8'));
    }
}
class_alias(Exception::class, 'sfException', false);