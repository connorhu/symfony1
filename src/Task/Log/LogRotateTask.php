 ...
 * @version SVN: $Id$
 */
class LogRotateTask extends BaseTask
{
    /**
    ...
    public function rotate($app, $env, $period = null, $history = null, $override = false)
    {
        ...
        $logs = Finder::type('file')->maxdepth(1)->name($logfile . '_*.log')->sort_by_name()->in($logdir . '/history');
        ...
        if (!$rotateOn || $rotateOn == $today || $override) {
            // create a lock file
            $lockFile = Config::get('sf_data_dir') . '/' . $app . '_' . $env . '-cli.lck';
            ...
            if (file_exists($srcLog)) {
                ...
                $newLogs = Finder::type('file')->maxdepth(1)->name($logfile . '_*.log')->sort_by_name()->in($logdir . '/history');
    ...
    {
        $this->addArguments(array(new CommandArgument('application', CommandArgument::REQUIRED, 'The application name'), new CommandArgument('env', CommandArgument::REQUIRED, 'The environment name')));
        $this->addOptions(array(new CommandOption('history', null, CommandOption::PARAMETER_REQUIRED, 'The maximum number of old log files to keep', self::DEF_HISTORY), new CommandOption('period', null, CommandOption::PARAMETER_REQUIRED, 'The period in days', self::DEF_PERIOD)));
        $this->namespace = 'log';
        $this->name = 'rotate';
...
class_alias(LogRotateTask::class, 'sfLogRotateTask', false);