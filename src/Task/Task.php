...
use Symfony1\Components\Command\CommandArgumentSet;
use Symfony1\Components\Command\CommandOptionSet;
...
use DOMDocument;
use Symfony1\Components\Command\CommandArgumentsException;
...
abstract class Task
{
     ...
     * Constructor.
     *
     * @param EventDispatcher $dispatcher An sfEventDispatcher instance
     * @param Formatter $formatter An sfFormatter instance
     */
    public function __construct(EventDispatcher $dispatcher, Formatter $formatter)
    {
     ...
     */
    public function initialize(EventDispatcher $dispatcher, Formatter $formatter)
    {
     ...
     *
     * @param CommandManager $commandManager An sfCommandManager instance
     * @param mixed $options The command line options
     *
     * @return int 0 if everything went fine, or an error code
     */
    public function runFromCLI(CommandManager $commandManager, $options = null)
    {
    ...
    {
        $commandManager = new CommandManager(new CommandArgumentSet($this->getArguments()), new CommandOptionSet($this->getOptions()));
     ...
     *
     * @return CommandArgument[] an array of sfCommandArgument objects
     */
     ...
     *
     * @param CommandArgument[] $arguments An array of arguments
     */
    ...
    {
        $this->arguments[] = new CommandArgument($name, $mode, $help, $default);
    }
    /**
     * Returns the options objects.
     *
     * @return CommandOption[] an array of sfCommandOption objects
     */
    ...
    {
        $this->options[] = new CommandOption($name, $shortcut, $mode, $help, $default);
    }
    /**
    ...
    */
    ...
    {
        $this->dispatcher->notify(new Event($this, 'command.log', array($this->formatter->formatSection($section, $message, $size, $style))));
    }
    /**
    ...
    */
    public function askAndValidate($question, ValidatorBase $validator, array $options = array())
    {
    ...
    protected function process(CommandManager $commandManager, $options)
    {
        $commandManager->process($options);
        if (!$commandManager->isValid()) {
            throw new CommandArgumentsException(sprintf("The execution of task \"%s\" failed.\n- %s", $this->getFullName(), implode("\n- ", $commandManager->getErrors())));
        }
    }
     ...
     * @return int 0 if everything went fine, or an error code
     */
    protected abstract function execute($arguments = array(), $options = array());
...
class_alias(Task::class, 'sfTask', false);