...
namespace Symfony1\Components\Task\App;
 ...
 * @version SVN: $Id$
 */
class AppRoutesTask extends BaseTask
{
    ...
    {
        $this->addArguments(array(new CommandArgument('application', CommandArgument::REQUIRED, 'The application name'), new CommandArgument('name', CommandArgument::OPTIONAL, 'A route name')));
        $this->namespace = 'app';
        $this->name = 'routes';
...
EOF;
    ...
    protected function outputRoute($application, $name)
    {
        ...
        if (!isset($this->routes[$name])) {
            throw new CommandException(sprintf('The route "%s" does not exist.', $name));
        }
...
class_alias(AppRoutesTask::class, 'sfAppRoutesTask', false);