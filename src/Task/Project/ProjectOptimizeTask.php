 ...
 * @version SVN: $Id$
 */
class ProjectOptimizeTask extends BaseTask
{
    /**
     * @see sfTask
     */
    ...
    {
        $this->addArguments(array(new CommandArgument('application', CommandArgument::REQUIRED, 'The application name'), new CommandArgument('env', CommandArgument::OPTIONAL, 'The environment name', 'prod')));
...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        $target = Config::get('sf_cache_dir') . '/' . $arguments['application'] . '/' . $arguments['env'] . '/config/configuration.php';
        $current_umask = umask();
        umask(00);
        ...
        Context::createInstance($this->configuration);
        ...
        foreach ($modules as $module) {
            ...
            try {
                ...
                $this->dispatcher->notifyUntil(new Event($e, 'application.throw_exception'));
    ...
    {
        $data = array();
        $finder = Finder::type('file')->name('*Helper.php');
        // module helpers
        foreach ($modules as $module) {
            $helpers = array();
    ...
    protected function findTemplates($modules)
    {
        $files = array();
        foreach ($modules as $module) {
            $files[$module] = Finder::type('file')->follow_link()->relative()->in($this->configuration->getTemplateDirs($module));
        }
        return $files;
    }
    ...
    {
        // application
        $dirs = array(Config::get('sf_app_module_dir'));
        // plugins
        $pluginSubPaths = $this->configuration->getPluginSubPaths(DIRECTORY_SEPARATOR . 'modules');
        $modules = array();
        foreach (Finder::type('dir')->maxdepth(0)->follow_link()->relative()->in($pluginSubPaths) as $module) {
            if (in_array($module, Config::get('sf_enabled_modules'))) {
                $modules[] = $module;
            }
        }
        // core modules
        $dirs[] = Config::get('sf_symfony_lib_dir') . '/controller';
        return array_unique(array_merge(Finder::type('dir')->maxdepth(0)->follow_link()->relative()->in($dirs), $modules));
    }
}
class_alias(ProjectOptimizeTask::class, 'sfProjectOptimizeTask', false);