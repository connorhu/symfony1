 ...
 * @version SVN: $Id$
 */
class ProjectDeployTask extends BaseTask
{
    ...
    {
        $this->addArguments(array(new CommandArgument('server', CommandArgument::REQUIRED, 'The server name')));
        $this->addOptions(array(new CommandOption('go', null, CommandOption::PARAMETER_NONE, 'Do the deployment'), new CommandOption('rsync-dir', null, CommandOption::PARAMETER_REQUIRED, 'The directory where to look for rsync*.txt files', 'config'), new CommandOption('rsync-options', null, CommandOption::PARAMETER_OPTIONAL, 'To options to pass to the rsync executable', '-azC --force --delete --progress')));
...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        $ini = Config::get('sf_config_dir') . '/properties.ini';
        if (!file_exists($ini)) {
            throw new CommandException('You must create a config/properties.ini file');
        }
        $properties = parse_ini_file($ini, true);
        if (!isset($properties[$env])) {
            throw new CommandException(sprintf('You must define the configuration for server "%s" in config/properties.ini', $env));
        }
        $properties = $properties[$env];
        if (!isset($properties['host'])) {
            throw new CommandException('You must define a "host" entry.');
        }
        if (!isset($properties['dir'])) {
            throw new CommandException('You must define a "dir" entry.');
        }
...
class_alias(ProjectDeployTask::class, 'sfProjectDeployTask', false);