 ...
 * @version SVN: $Id$
 */
class TestAllTask extends TestBaseTask
{
    /**
     * @see sfTask
     */
    ...
    {
        $this->addOptions(array(new CommandOption('only-failed', 'f', CommandOption::PARAMETER_NONE, 'Only run tests that failed last time'), new CommandOption('full-output', 'o', CommandOption::PARAMETER_NONE, 'Display full path for the test'), new CommandOption('xml', null, CommandOption::PARAMETER_REQUIRED, 'The file name for the JUnit compatible XML log file')));
        $this->namespace = 'test';
        $this->name = 'all';
...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        $h->base_dir = Config::get('sf_test_dir');
        $status = false;
        $statusFile = Config::get('sf_cache_dir') . '/.test_all_status';
...
class_alias(TestAllTask::class, 'sfTestAllTask', false);