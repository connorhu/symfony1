...
namespace Symfony1\Components\Task\Cache;
 ...
 * @version SVN: $Id$
 */
class CacheClearTask extends BaseTask
{
    protected $config;
    public function getFactoriesConfiguration(ApplicationConfiguration $appConfiguration)
    {
        ...
        if (!isset($this->config[$app][$env])) {
            $this->config[$app][$env] = FactoryConfigHandler::getConfiguration($appConfiguration->getConfigPaths('config/factories.yml'));
        }
    ...
    {
        $this->addOptions(array(new CommandOption('app', null, CommandOption::PARAMETER_OPTIONAL, 'The application name', null), new CommandOption('env', null, CommandOption::PARAMETER_OPTIONAL, 'The environment', null), new CommandOption('type', null, CommandOption::PARAMETER_OPTIONAL, 'The type', 'all')));
...
EOF;
    ...
    {
        if (!Config::get('sf_cache_dir') || !is_dir(Config::get('sf_cache_dir'))) {
            throw new ExceptionException(sprintf('Cache directory "%s" does not exist.', Config::get('sf_cache_dir')));
        }
        // finder to find directories (1 level) in a directory
        $dirFinder = Finder::type('dir')->discard('.*')->maxdepth(0)->relative();
        // iterate through applications
        $apps = null === $options['app'] ? $dirFinder->in(Config::get('sf_apps_dir')) : array($options['app']);
        foreach ($apps as $app) {
            $this->checkAppExists($app);
            if (!is_dir(Config::get('sf_cache_dir') . '/' . $app)) {
                continue;
            }
            // iterate through environments
            $envs = null === $options['env'] ? $dirFinder->in(Config::get('sf_cache_dir') . '/' . $app) : array($options['env']);
            foreach ($envs as $env) {
                if (!is_dir(Config::get('sf_cache_dir') . '/' . $app . '/' . $env)) {
                    continue;
                }
                ...
                $this->lock($app, $env);
                $event = $appConfiguration->getEventDispatcher()->notifyUntil(new Event($this, 'task.cache.clear', array('app' => $appConfiguration, 'env' => $env, 'type' => $options['type'])));
        ...
        if (null === $options['app'] && 'all' == $options['type']) {
            $this->getFilesystem()->remove(Finder::type('file')->discard('.*')->in(Config::get('sf_cache_dir')));
        }
    }
    ...
    protected function clearAllCache(ApplicationConfiguration $appConfiguration)
    {
    ...
    protected function clearConfigCache(ApplicationConfiguration $appConfiguration)
    {
        $subDir = Config::get('sf_cache_dir') . '/' . $appConfiguration->getApplication() . '/' . $appConfiguration->getEnvironment() . '/config';
        if (is_dir($subDir)) {
            // remove cache files
            $this->getFilesystem()->remove(Finder::type('file')->discard('.*')->in($subDir));
        }
    }
    protected function clearI18NCache(ApplicationConfiguration $appConfiguration)
    {
    ...
    protected function clearRoutingCache(ApplicationConfiguration $appConfiguration)
    {
        $config = $this->getFactoriesConfiguration($appConfiguration);
    ...
    protected function clearTemplateCache(ApplicationConfiguration $appConfiguration)
    {
        $config = $this->getFactoriesConfiguration($appConfiguration);
    ...
    protected function clearModuleCache(ApplicationConfiguration $appConfiguration)
    {
        $subDir = Config::get('sf_cache_dir') . '/' . $appConfiguration->getApplication() . '/' . $appConfiguration->getEnvironment() . '/modules';
        if (is_dir($subDir)) {
            // remove cache files
            $this->getFilesystem()->remove(Finder::type('file')->discard('.*')->in($subDir));
        }
    ...
    {
        return Config::get('sf_data_dir') . '/' . $app . '_' . $env . '-cli.lck';
    }
}
class_alias(CacheClearTask::class, 'sfCacheClearTask', false);