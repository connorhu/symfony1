 ...
 * @version SVN: $Id$
 */
class GenerateAppTask extends GeneratorBaseTask
{
    /**
     * @see sfTask
     */
    ...
    {
        $this->addArguments(array(new CommandArgument('app', CommandArgument::REQUIRED, 'The application name')));
        $this->addOptions(array(new CommandOption('escaping-strategy', null, CommandOption::PARAMETER_REQUIRED, 'Output escaping strategy', true), new CommandOption('csrf-secret', null, CommandOption::PARAMETER_REQUIRED, 'Secret to use for CSRF protection', true)));
...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        if (!preg_match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $app)) {
            throw new CommandException(sprintf('The application name "%s" is invalid.', $app));
        }
        $appDir = Config::get('sf_apps_dir') . '/' . $app;
        if (is_dir($appDir)) {
            throw new CommandException(sprintf('The application "%s" already exists.', $appDir));
        }
        if (is_readable(Config::get('sf_data_dir') . '/skeleton/app')) {
            $skeletonDir = Config::get('sf_data_dir') . '/skeleton/app';
        } else {
        ...
        $indexName = 'index';
        $firstApp = !file_exists(Config::get('sf_web_dir') . '/index.php');
        if (!$firstApp) {
            $indexName = $app;
        }
        ...
        $finder = Finder::type('file')->name('settings.yml');
        $this->getFilesystem()->replaceTokens($finder->in($appDir . '/config'), '##', '##', array('NO_SCRIPT_NAME' => $firstApp ? 'true' : 'false', 'CSRF_SECRET' => YamlInline::dump(YamlInline::parseScalar($options['csrf-secret'])), 'ESCAPING_STRATEGY' => YamlInline::dump((bool) YamlInline::parseScalar($options['escaping-strategy'])), 'USE_DATABASE' => Config::has('sf_orm') ? 'true' : 'false'));
        $this->getFilesystem()->copy($skeletonDir . '/web/index.php', Config::get('sf_web_dir') . '/' . $indexName . '.php');
        $this->getFilesystem()->copy($skeletonDir . '/web/index.php', Config::get('sf_web_dir') . '/' . $app . '_dev.php');
        $this->getFilesystem()->replaceTokens(Config::get('sf_web_dir') . '/' . $indexName . '.php', '##', '##', array('APP_NAME' => $app, 'ENVIRONMENT' => 'prod', 'IS_DEBUG' => 'false', 'IP_CHECK' => ''));
        $this->getFilesystem()->replaceTokens(Config::get('sf_web_dir') . '/' . $app . '_dev.php', '##', '##', array('APP_NAME' => $app, 'ENVIRONMENT' => 'dev', 'IS_DEBUG' => 'true', 'IP_CHECK' => '// this check prevents access to debug front controllers that are deployed by accident to production servers.' . PHP_EOL . '// feel free to remove this, extend it or make something more sophisticated.' . PHP_EOL . 'if (!in_array(@$_SERVER[\'REMOTE_ADDR\'], array(\'127.0.0.1\', \'::1\')))' . PHP_EOL . '{' . PHP_EOL . '  die(\'You are not allowed to access this file. Check \'.basename(__FILE__).\' for more information.\');' . PHP_EOL . '}' . PHP_EOL));
        ...
        $fixPerms->run();
        // Create test dir
        $this->getFilesystem()->mkdirs(Config::get('sf_test_dir') . '/functional/' . $app);
    }
}
class_alias(GenerateAppTask::class, 'sfGenerateAppTask', false);