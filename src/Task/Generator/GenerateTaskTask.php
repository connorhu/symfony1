 ...
 */
class GenerateTaskTask extends BaseTask
{
    /**
     * @see sfTask
     */
    ...
    {
        $this->addArguments(array(new CommandArgument('task_name', CommandArgument::REQUIRED, 'The task name (can contain namespace)')));
        $this->addOptions(array(new CommandOption('dir', null, CommandOption::PARAMETER_REQUIRED, 'The directory to create the task in', 'lib/task'), new CommandOption('use-database', null, CommandOption::PARAMETER_REQUIRED, 'Whether the task needs model initialization to access database', Config::get('sf_orm')), new CommandOption('brief-description', null, CommandOption::PARAMETER_REQUIRED, 'A brief task description (appears in task list)')));
...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        if (!preg_match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $taskClassName)) {
            throw new CommandException(sprintf('The task class name "%s" is invalid.', $taskClassName));
        }
...
HED;
        $useDatabase = Toolkit::literalize($options['use-database']);
        $defaultConnection = is_string($useDatabase) ? $useDatabase : Config::get('sf_orm');
        if ($useDatabase) {
            $content = <<<HED
<?php
...
HED;
        ...
        if (!is_readable(Config::get('sf_root_dir') . '/' . $options['dir'])) {
            $this->getFilesystem()->mkdirs($options['dir']);
        }
        $taskFile = Config::get('sf_root_dir') . '/' . $options['dir'] . '/' . $taskClassName . '.class.php';
        if (is_readable($taskFile)) {
            throw new CommandException(sprintf('A "%s" task already exists in "%s".', $taskName, $taskFile));
        }
        $this->logSection('task', sprintf('Creating "%s" task file', $taskFile));
        file_put_contents($taskFile, $content);
    }
}
class_alias(GenerateTaskTask::class, 'sfGenerateTaskTask', false);