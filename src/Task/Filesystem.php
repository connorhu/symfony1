...
class Filesystem
{
     ...
     */
    public function __construct(EventDispatcher $dispatcher = null, Formatter $formatter = null)
    {
     ...
     */
    public function chmod($files, $mode, $umask = 00)
    {
        $currentUmask = umask();
        umask($umask);
        if (!is_array($files)) {
     ...
     * @param Finder $finder An sfFinder instance
    ...
    protected function logSection($section, $message, $size = null)
    {
        ...
        $this->dispatcher->notify(new Event($this, 'command.log', array($message)));
    }
    /**
...
class_alias(Filesystem::class, 'sfFilesystem', false);