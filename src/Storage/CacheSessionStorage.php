...
*/
class CacheSessionStorage extends Storage
{
    /**
     * @var string
     */
    ...
    */
    public function initialize($options = array())
    {
        ...
        if (empty($this->id)) {
            ...
            if (Config::get('sf_logging_enabled')) {
                $this->dispatcher->notify(new Event($this, 'application.log', array('New session created')));
            }
            ...
            if (Config::get('sf_logging_enabled')) {
                $this->dispatcher->notify(new Event($this, 'application.log', array('Restored previous session')));
            }
        }
        session_id($this->id);
    ...
    public function expire()
    {
        ...
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array('new session created due to expiraton')));
        }
    }
    /**
    ...
    public function shutdown()
    {
        ...
        if (true === $this->dataChanged) {
            $this->cache->set($this->id, serialize($this->data));
            if (Config::get('sf_logging_enabled')) {
                $this->dispatcher->notify(new Event($this, 'application.log', array('Storing session to cache')));
            }
        }
    }
}
class_alias(CacheSessionStorage::class, 'sfCacheSessionStorage', false);