...
abstract class Response implements Serializable
{
    ...
    {
        $event = $this->dispatcher->notifyUntil(new Event($this, 'response.method_not_found', array('method' => $method, 'arguments' => $arguments)));
    ...
    {
        $event = $this->dispatcher->filter(new Event($this, 'response.filter_content'), $this->getContent());
        $content = $event->getReturnValue();
        if ($this->options['logging']) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Send content (%s o)', strlen($content)))));
        }
        echo $content;
    }
    /**
...
class_alias(Response::class, 'sfResponse', false);