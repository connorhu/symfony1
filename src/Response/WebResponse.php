...
namespace Symfony1\Components\Response;
...
class WebResponse extends Response
{
    ...
    */
    public function sendHttpHeaders()
    {
        ...
        if ($this->options['logging']) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Send status "%s"', $status))));
        }
        // headers
        ...
        foreach ($this->headers as $name => $value) {
            ...
            if ('' != $value && $this->options['logging']) {
                $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Send header "%s: %s"', $name, $value))));
            }
        }
        // cookies
        foreach ($this->cookies as $cookie) {
            ...
            if ($this->options['logging']) {
                $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Send cookie "%s": "%s"', $cookie['name'], $cookie['value']))));
            }
        }
    ...
    public function send()
    {
        ...
        if (function_exists('fastcgi_finish_request')) {
            $this->dispatcher->notify(new Event($this, 'response.fastcgi_finish_request'));
    ...
    */
     ...
     *
     * @param WebResponse $response An sfWebResponse instance
     */
    public function copyProperties(WebResponse $response)
    {
     ...
     * @param WebResponse $response An sfWebResponse instance
     */
    public function merge(WebResponse $response)
    {
        foreach ($this->getPositions() as $position) {
...
class_alias(WebResponse::class, 'sfWebResponse', false);