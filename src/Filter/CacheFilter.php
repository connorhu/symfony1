        ...
        if (Config::get('sf_etag')) {
            ...
            if ($this->request->getHttpHeader('IF_NONE_MATCH') == $etag) {
                ...
                if (Config::get('sf_logging_enabled')) {
                    $this->context->getEventDispatcher()->notify(new Event($this, 'application.log', array('ETag matches If-None-Match (send 304)')));
                }
            }
        }
        // conditional GET support
        // never in debug mode
        if ($this->response->hasHttpHeader('Last-Modified') && (!Config::get('sf_debug') || Config::get('sf_test'))) {
            $lastModified = $this->response->getHttpHeader('Last-Modified');
            if ($this->request->getHttpHeader('IF_MODIFIED_SINCE') == $lastModified) {
                $this->response->setStatusCode(304);
                $this->response->setHeaderOnly(true);
                if (Config::get('sf_logging_enabled')) {
                    $this->context->getEventDispatcher()->notify(new Event($this, 'application.log', array('Last-Modified matches If-Modified-Since (send 304)')));
                }
            }
        }
    }
}
class_alias(CacheFilter::class, 'sfCacheFilter', false);