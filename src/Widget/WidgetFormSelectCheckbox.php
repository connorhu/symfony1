 ...
 * @version SVN: $Id$
 */
class WidgetFormSelectCheckbox extends WidgetFormChoiceBase
{
    /**
     * Renders the widget.
     *
    ...
    */
    ...
    protected function formatChoices($name, $value, $choices, $attributes)
    {
        $inputs = array();
        foreach ($choices as $key => $option) {
            $baseAttributes = array('name' => $name, 'type' => 'checkbox', 'value' => self::escapeOnce($key), 'id' => $id = $this->generateId($name, self::escapeOnce($key)));
            if (is_array($value) && in_array((string) $key, $value) || is_string($value) && (string) $key == (string) $value) {
                $baseAttributes['checked'] = 'checked';
            }
            $inputs[$id] = array('input' => $this->renderTag('input', array_merge($baseAttributes, $attributes)), 'label' => $this->renderContentTag('label', self::escapeOnce($option), array('for' => $id)));
        }
        return call_user_func($this->getOption('formatter'), $this, $inputs);
    }
}
class_alias(WidgetFormSelectCheckbox::class, 'sfWidgetFormSelectCheckbox', false);