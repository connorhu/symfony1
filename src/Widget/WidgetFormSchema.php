 ...
 * @version SVN: $Id$
 */
class WidgetFormSchema extends WidgetForm implements ArrayAccess
{
    ...
    */
     ...
     *
     * @return Widget The current widget instance
     */
    ...
    public function getDefaults()
    {
        $defaults = array();
        foreach ($this->fields as $name => $widget) {
            $defaults[$name] = $widget instanceof WidgetFormSchema ? $widget->getDefaults() : $widget->getDefault();
        }
        return $defaults;
    }
    /**
     ...
     * @param WidgetFormSchemaFormatter $formatter An sfWidgetFormSchemaFormatter instance
     *
     * @return Widget The current widget instance
     */
    public function addFormFormatter($name, WidgetFormSchemaFormatter $formatter)
    {
    ...
    */
     ...
     *
     * @return WidgetFormSchemaFormatter sfWidgetFormSchemaFormatter instance
     *
    ...
    */
    ...
    public function renderField($name, $value = null, $attributes = array(), $errors = array())
    {
        ...
        if ($errors && $widget instanceof WidgetFormSchema && !$errors instanceof ValidatorErrorSchema) {
            $schema = new ValidatorErrorSchema($errors->getValidator());
    ...
    public function render($name, $values = array(), $attributes = array(), $errors = array())
    {
        ...
        foreach ($this->positions as $name) {
            ...
            if ($widget instanceof WidgetForm && $widget->isHidden()) {
                ...
                $label = $widget instanceof WidgetFormSchema ? $formFormat->generateLabelName($name) : $formFormat->generateLabel($name);
                $error = $widget instanceof WidgetFormSchema ? array() : $error;
    ...
    public function getGlobalErrors($errors)
    {
        ...
        foreach ($this->positions as $name) {
             ...
             * @var WidgetForm[] $this
             */
            if ($this[$name] instanceof WidgetForm && $this[$name]->isHidden()) {
     ...
     * @param string $name The field name
     *
     * @return (Widget | null) The sfWidget instance associated with the given name, null if it does not exist
     */
    #[ReturnTypeWillChange]
     ...
     *
     * @param string $name The field name
     * @param Widget $widget An sfWidget instance
     *
     ...
     *
     * @return Widget[] An array of sfWidget instance
     */
    ...
    */
    public function moveField($field, $action, $pivot = null)
    {
        ...
        switch ($action) {
            case WidgetFormSchema::FIRST:
                array_unshift($this->positions, $field);
                break;
            case WidgetFormSchema::LAST:
                $this->positions[] = $field;
                break;
            case WidgetFormSchema::BEFORE:
                if (null === $pivot) {
                    throw new LogicException(sprintf('Unable to move field "%s" without a relative field.', $field));
                }
                $this->positions = array_merge(array_slice($this->positions, 0, $pivotPosition), array($field), array_slice($this->positions, $pivotPosition));
                break;
            case WidgetFormSchema::AFTER:
                if (null === $pivot) {
                    throw new LogicException(sprintf('Unable to move field "%s" without a relative field.', $field));
                }
                $this->positions = array_merge(array_slice($this->positions, 0, $pivotPosition + 1), array($field), array_slice($this->positions, $pivotPosition + 1));
                break;
            default:
...
class_alias(WidgetFormSchema::class, 'sfWidgetFormSchema', false);