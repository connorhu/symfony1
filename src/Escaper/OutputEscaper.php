...
abstract class OutputEscaper
{
    ...
    */
    public static function escape($escapingMethod, $value)
    {
        ...
        if (is_array($value)) {
            return new OutputEscaperArrayDecorator($escapingMethod, $value);
        }
        if (is_object($value)) {
            if ($value instanceof OutputEscaper) {
            ...
            if ($value instanceof OutputEscaperSafe) {
                // do not escape objects marked as safe
                // return the original object
                return $value->getValue();
            }
            if ($value instanceof Traversable) {
                return new OutputEscaperIteratorDecorator($escapingMethod, $value);
            }
            return new OutputEscaperObjectDecorator($escapingMethod, $value);
        }
    ...
    public static function unescape($value)
    {
        ...
        if (is_scalar($value)) {
            return html_entity_decode($value, ENT_QUOTES, Config::get('sf_charset'));
        }
        if (is_array($value)) {
        ...
        if (is_object($value)) {
            return $value instanceof OutputEscaper ? $value->getRawValue() : $value;
        }
        return $value;
    }
    /**
...
class_alias(OutputEscaper::class, 'sfOutputEscaper', false);