...
class PHPView extends View
{
    ...
    public function render()
    {
        ...
        if (null === $content) {
            ...
            if (Config::get('sf_cache') && null !== $uri) {
    ...
    protected function loadCoreAndStandardHelpers()
    {
        ...
        $coreHelpersLoaded = 1;
        $helpers = array_unique(array_merge(array('Helper', 'Url', 'Asset', 'Tag', 'Escaping'), Config::get('sf_standard_helpers')));
    ...
    protected function renderFile($_sfFile)
    {
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Render "%s"', $_sfFile))));
        }
    ...
    {
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Decorate content with "%s/%s"', $this->getDecoratorDirectory(), $this->getDecoratorTemplate()))));
        }
        // set the decorator content as an attribute
        $attributeHolder = $this->attributeHolder;
        $this->attributeHolder = $this->initializeAttributeHolder(array('sf_content' => new OutputEscaperSafe($content)));
        ...
        if (!is_readable($this->getDecoratorDirectory() . '/' . $this->getDecoratorTemplate())) {
            throw new RenderException(sprintf('The decorator template "%s" does not exist or is unreadable in "%s".', $this->decoratorTemplate, $this->decoratorDirectory));
        }
...
class_alias(PHPView::class, 'sfPHPView', false);