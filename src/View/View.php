...
use Symfony1\Components\Escaper\OutputEscaper;
...
abstract class View
{
    ...
    {
        $event = $this->dispatcher->notifyUntil(new Event($this, 'view.method_not_found', array('method' => $method, 'arguments' => $arguments)));
    ...
    public function initialize($context, $moduleName, $actionName, $viewName)
    {
        ...
        OutputEscaper::markClassesAsSafe(array('sfForm', 'sfFormField', 'sfFormFieldSchema', 'sfModelGeneratorHelper'));
        $this->attributeHolder = $this->initializeAttributeHolder();
        $this->parameterHolder = new ParameterHolder();
        $this->parameterHolder->add(Config::get('mod_' . strtolower($moduleName) . '_view_param', array()));
        ...
        $this->dispatcher->notify(new Event($this, 'view.configure_format', array('format' => $format, 'response' => $context->getResponse(), 'request' => $context->getRequest())));
     ...
     *
     * @return ParameterHolder The attribute parameter holder
     */
    ...
    */
    ...
    {
        return new ViewParameterHolder($this->dispatcher, $attributes, array('escaping_method' => Config::get('sf_escaping_method'), 'escaping_strategy' => Config::get('sf_escaping_strategy')));
    }
    /**
    ...
    */
    protected function preRenderCheck()
    {
        if (null === $this->template) {
            ...
            throw new RenderException('A template has not been set.');
        }
        if (!is_readable($this->directory . '/' . $this->template)) {
            // 404?
            if ('404' == $this->context->getResponse()->getStatusCode()) {
                // use default exception templates
                $this->template = Exception::getTemplatePathForError($this->context->getRequest()->getRequestFormat(), false);
                ...
                throw new RenderException(sprintf('The template "%s" does not exist or is unreadable in "%s".', $this->template, $this->directory));
            }
        }
    }
}
class_alias(View::class, 'sfView', false);