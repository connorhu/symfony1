...
namespace Symfony1\Components\View;

use Symfony1\Components\Cache\Cache;
...
class ViewCacheManager
{
     ...
     * @see initialize()
     */
    public function __construct($context, Cache $cache, $options = array())
    {
     ...
     * @param Cache $cache An sfCache instance
     */
    public function initialize($context, Cache $cache, $options = array())
    {
     ...
     *
     * @return Context The sfContext instance
     */
     ...
     *
     * @return Cache The current cache object
     */
    ...
    */
    ...
    {
        if ($callable = Config::get('sf_cache_namespace_callable')) {
            if (!is_callable($callable)) {
                throw new Exception(sprintf('"%s" cannot be called as a function.', var_export($callable, true)));
            }
        ...
        if (0 === strpos($internalUri, '@') && false === strpos($internalUri, '@sf_cache_partial')) {
            throw new Exception('A cache key cannot be generated for an internal URI using the @rule syntax');
        }
        $cacheKey = '';
        if ($this->isContextual($internalUri)) {
            ...
            if (!$contextualPrefix) {
                ...
                if (!isset($params['module']) || !isset($params['action'])) {
                    $params['module'] = Config::get('sf_error_404_module');
                    $params['action'] = Config::get('sf_error_404_action');
                }
        ...
        return strtr($cacheKey, array('/.' => '/_.', '/_' => '/__', '\\.' => '\\_.', '\\_' => '\\__'));
    }
    /**
    ...
    public function addCache($moduleName, $actionName, $options = array())
    {
        ...
        $this->cacheConfig[$moduleName][$actionName] = array('withLayout' => isset($options['withLayout']) ? $options['withLayout'] : false, 'lifeTime' => $options['lifeTime'], 'clientLifeTime' => isset($options['clientLifeTime']) ? $options['clientLifeTime'] : $options['lifeTime'], 'contextual' => isset($options['contextual']) ? $options['contextual'] : false, 'vary' => isset($options['vary']) ? $options['vary'] : array());
    }
    /**
    ...
    */
    ...
    {
        if ($this->request instanceof WebRequest && !$this->request->isMethod(Request::GET)) {
            return false;
        }
        list($route_name, $params) = $this->controller->convertUrlStringToParameters($internalUri);
    ...
    public function get($internalUri)
    {
        ...
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Cache for "%s" %s', $internalUri, null !== $retval ? 'exists' : 'does not exist'))));
        }
        return $retval;
    }
    /**
    ...
    public function set($data, $internalUri)
    {
        ...
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Save cache for "%s"', $internalUri))));
        }
        return true;
    }
    /**
    ...
    {
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array(sprintf('Remove cache for "%s"', $internalUri))));
        }
    ...
    public function computeCacheKey(array $parameters)
    {
        ...
        if (Config::get('sf_logging_enabled')) {
            $this->dispatcher->notify(new Event($this, 'application.log', array('Generate cache key')));
        }
        ksort($parameters);
    ...
    */
    ...
    public function getPartialCache($module, $action, $cacheKey)
    {
        ...
        if (Config::get('sf_web_debug')) {
            $content = $this->dispatcher->filter(new Event($this, 'view.cache.filter_content', array('response' => $this->context->getResponse(), 'uri' => $uri, 'new' => false)), $content)->getReturnValue();
        }
        return $content;
    }
    /**
    ...
    public function setPartialCache($module, $action, $cacheKey, $content)
    {
        ...
        if ($saved && Config::get('sf_web_debug')) {
            $content = $this->dispatcher->filter(new Event($this, 'view.cache.filter_content', array('response' => $this->context->getResponse(), 'uri' => $uri, 'new' => true)), $content)->getReturnValue();
        }
        return $content;
    }
    /**
    ...
    {
        if (View::RENDER_CLIENT != $this->controller->getRenderMode()) {
            return;
        }
        // save content in cache
        $saved = $this->set(serialize($this->context->getResponse()), $uri);
        if ($saved && Config::get('sf_web_debug')) {
            $content = $this->dispatcher->filter(new Event($this, 'view.cache.filter_content', array('response' => $this->context->getResponse(), 'uri' => $uri, 'new' => true)), $this->context->getResponse()->getContent())->getReturnValue();
    ...
    public function getPageCache($uri)
    {
        ...
        if (View::RENDER_VAR == $this->controller->getRenderMode()) {
            ...
            if (Config::get('sf_web_debug')) {
                $content = $this->dispatcher->filter(new Event($this, 'view.cache.filter_content', array('response' => $this->context->getResponse(), 'uri' => $uri, 'new' => false)), $this->context->getResponse()->getContent())->getReturnValue();
                $this->context->getResponse()->setContent($content);
            }
        }
        return true;
    ...
    */
     ...
     */
    public function decorateContentWithDebug(Event $event, $content)
    {
      ...
      <div style="height: 16px; padding: 2px"><a href="#" onclick="sfWebDebugToggle(\'sub_main_info_' . $id . '\'); return false;"><strong>cache information</strong></a>&nbsp;<a href="#" onclick="sfWebDebugToggle(\'sub_main_' . $id . '\'); document.getElementById(\'main_' . $id . '\').style.border = \'none\'; return false;">' . image_tag(Config::get('sf_web_debug_web_dir') . '/images/close.png', array('alt' => 'close')) . '</a>&nbsp;</div>
        <div style="padding: 2px; display: none" id="sub_main_info_' . $id . '">
        [uri]&nbsp;' . htmlspecialchars($event['uri'], ENT_QUOTES, Config::get('sf_charset')) . '<br />
        [key&nbsp;for&nbsp;cache]&nbsp;' . htmlspecialchars($cacheKey, ENT_QUOTES, Config::get('sf_charset')) . '<br />
    ...
    {
        if (!isset($params['module']) || !isset($params['action'])) {
            throw new Exception('A cache key must contain both a module and an action parameter');
        }
    ...
    protected function ignore()
    {
        ...
        if (Config::get('sf_debug') && $this->context->getRequest()->getAttribute('sf_ignore_cache')) {
            if (Config::get('sf_logging_enabled')) {
                $this->dispatcher->notify(new Event($this, 'application.log', array('Discard cache')));
            }
            return true;
        }
        return false;
...
class_alias(ViewCacheManager::class, 'sfViewCacheManager', false);