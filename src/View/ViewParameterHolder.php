 ...
 * @version SVN: $Id$
 */
class ViewParameterHolder extends ParameterHolder
{
     ...
     * Constructor.
     */
    public function __construct(EventDispatcher $dispatcher, $parameters = array(), $options = array())
    {
    ...
    public function __unserialize($data)
    {
        ...
        $this->initialize(Context::hasInstance() ? Context::getInstance()->getEventDispatcher() : new EventDispatcher());
    ...
    *
    * @throws InitializationException if an error occurs while initializing this view parameter holder
    */
    public function initialize(EventDispatcher $dispatcher, $parameters = array(), $options = array())
    {
    ...
    {
        $event = $this->dispatcher->filter(new Event($this, 'template.filter_parameters'), $this->getAll());
        $parameters = $event->getReturnValue();
        $attributes = array();
        if ($this->isEscaped()) {
            $attributes['sf_data'] = OutputEscaper::escape($this->getEscapingMethod(), $parameters);
            foreach ($attributes['sf_data'] as $key => $value) {
                $attributes[$key] = $value;
            }
        } elseif (in_array($this->getEscaping(), array('off', false), true)) {
            $attributes = $parameters;
            $attributes['sf_data'] = OutputEscaper::escape(ESC_RAW, $parameters);
        } else {
...
class_alias(ViewParameterHolder::class, 'sfViewParameterHolder', false);